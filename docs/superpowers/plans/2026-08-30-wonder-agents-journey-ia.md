...
| D23 | **The header must not change height between frames.** Adding the breadcrumb row must not grow the page or introduce vertical scroll. Height comes from flexbox (`h-screen flex flex-col`, `flex-1 overflow-y-auto` body), never from `calc(100vh - <fixed>)` arithmetic. | User-reported bug: the header "jumped" and added scroll once breadcrumbs appeared. Any later change that reintroduces fixed-pixel header math is a regression. |
| D24 | **Breadcrumbs are prominent.** 13px, `--wp-ink-2` for inactive crumbs, generous gap and padding — deliberately stronger than the quiet-small-text treatment the plan originally specified. | User asked for more prominent breadcrumbs after reviewing the build. Overrides the "quiet line of small text" wording elsewhere in this plan. |
| D26 | **Home presents three equal primary actions:** `צור סוכן`, `בנה פלאגין`, `התחל שיחה`. The plugin panel's line is `ארוז מיומנויות, כלים וידע, כדי לתת לכל סוכן`. | User override. The controller had specified the plugin entry as visibly subordinate to protect the agent journey's primacy; that reasoning is withdrawn. |
| D27 | **The sidebar carries both `סוכן חדש` and `פלאגין חדש` as buttons**, of equal weight. | User direction. Both need the same `createItem` wiring. |
| D28 | **The catalog group is named `קטלוג` and is never collapsible.** No toggle, no chevron, no open/closed state — the items are always visible under a plain group label. | User direction. This supersedes D15, which kept the group collapsed except when the current view was a catalog. |
| D25 | **One step style across every workspace.** Whatever step treatment the agent journey settles on — the progress rail, the step nav, the primary `המשך` action — is used identically by every frame: agents, plugins, subagents, skills, tools, knowledge and evaluations. No frame keeps the old plain tab row. | User direction. The step sets in `wonder-platform-resource-fields.js` (`stepsFor`, `toolSteps`, `evalSteps`) must adopt it too, not only the workspace-backed frames. Step *content* stays per-resource; the *chrome* is uniform. |
