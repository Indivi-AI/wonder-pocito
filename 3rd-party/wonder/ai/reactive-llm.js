...
const getProviderConfig = (modelString, proxyAlias) => {
...
export async function fetchItemsFromLLMReactiveP(args) {
  ...
  if (args.hedgeAfterMs && !replay) return new Promise(resolve => {   // tail-latency hedge: a duplicate request after hedgeAfterMs, the first answer wins
    const timer = setTimeout(() => fetchItemsFromLLMReactiveP({...args, hedgeAfterMs: 0, goal: `${args.goal} (hedge)`}).then(resolve), args.hedgeAfterMs)
    fetchItemsFromLLMReactiveP({...args, hedgeAfterMs: 0}).then(res => (clearTimeout(timer), resolve(res)))
  })
