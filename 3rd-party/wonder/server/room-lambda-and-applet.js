...
const uiSource = appletSpec.liveRepo
  ? 'live-repo (localhost static mappings from package.json)'
...
const appletOgUrl = (req, shareKey) => {
  const url = `${req.hostname === 'localhost' ? 'http' : 'https'}://${req.get('host')}${req.path}`
  return `${url}${shareKey && req.query.share === shareKey ? `?share=${encodeURIComponent(shareKey)}` : ''}`
}
const ogTags = b => [
...
export async function serveAppletPage(spec, res, localImports) {
  ...
  // shareImports = the publishing repo's import map (served url layout), written into the def by uploadRoomApplet; older defs use the fixed map
  const imports = localImports ? localImports : spec.shareImports ? {
    ...Object.fromEntries(Object.entries(spec.shareImports).map(([alias, url]) => [alias, shareBase + url])), '@jb6/react/lib/': runtimeBase
  } : spec.clientCodeWUrl ? {
  ...
  const { og = [], shareImports, ...clientSpec } = spec   // og = raw branding sources (room, applet), server-only — not shipped to the client
...
export function setupRoomLambdaAndApplet(app) {
  ...
  const appletRoute = signed => async (req, res) => {
    try {
      ...
      const ogUrl = appletOgUrl(req, applet.og?.shareKey)
      ...
      const spec = { cmpId: applet.cmpId, urlsToLoad: applet.urlsToLoad, roomWUrl,
        appletV: applet.appletV, clientCodeWUrl: applet.clientCodeWUrl, shareImports: applet.shareImports,
        noAuth, og }
