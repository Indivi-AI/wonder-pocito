...
import { setupWfetch } from './wfetch.js'
import { serveAppletPage } from './room-lambda-and-applet.js'
import { setupLiveRepoRoomApplet, setupLiveRepoDevApplet } from './room-lambda-and-applet-live-repo.js'

const studio = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../studio')
...
app.get('/studio/tests.html', async (_, res) => {
  const html = await fs.readFile(path.join(studio, 'tests.html'), 'utf8')
...
await import(pathToFileURL(path.join(root, '.jb6/mcp.js')).href)
...
setupWfetch(app)
const {setupLlmProxyRoute} = llmProxyMode === 'onprem' ? await import('./llm-proxy-onprem.js') : await import('./llm-proxy.js')
