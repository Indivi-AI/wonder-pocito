...
import { repoLayout } from './repo-layout.js'

const asJbComp = c => typeof c == 'string' ? coreUtils.compByFullId(c) : c?.[Symbol.for('asJbComp')] || (c?.$location ? c : null)

const importsFromLoadedLocations = (profile, { repoRoot, toDisk }) => {
  const seen = new Set(), paths = []
  let missing = false
  const abs = p => p.startsWith('/') ? toDisk(p) : `${repoRoot}/${p}`   // $location.path is a served url, a disk path or repo-relative
  const walkComp = c => {
...
coreUtils.runUnPackagedInLiveRepo = async (ctx, compToRun) => {
  ...
  const profile = coreUtils.tgpProfileToJson(compToRun.profile), layout = await repoLayout()
  await coreUtils.calcJb6RepoRootAndImportMapsInCli()
  const fast = importsFromLoadedLocations(compToRun.profile, layout)
  const imp = fast || await coreUtils.calcImportsForProfile(profile, { entryPointPaths: [await coreUtils.resolveDeveloperEntryPoint(ctx)], ctx })
