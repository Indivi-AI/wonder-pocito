...
const runtimeLibUrl = '/jb6_packages/react/lib/'   // the versioned React runtime is served from the CDN (clientCode runtime), never from a share
// published layout = served url layout (repoLayout): a snapshot file lives at <share>/<served url>, e.g. <share>/jb6_packages/core/index.js
const servedInputs = (files, baseDir, path, toUrl) => files.filter(f => !f.includes('node_modules') && f !== '<stdin>')
  .map(file => ({ file, url: toUrl(path.resolve(baseDir, file)) })).filter(({ url }) => url)
...
export async function uploadCompDependencies(urlsToLoad) {
  ...
  const { repoRoot: baseDir, imports, toDisk, toUrl, aliasDirs } = await repoLayout(), onDisk = spec => toDisk(spec) || path.resolve(baseDir, spec)
  ...
  const sourceScopes = Object.keys(aliasDirs)
  const assets = new Set()
  const result = await esbuild.build({
    stdin: { contents: entries.map(f => `import '${onDisk(f)}'`).join('\n'), resolveDir: baseDir }, absWorkingDir: baseDir,
    bundle: true, write: false, metafile: true, format: 'esm', platform: 'browser', logLevel: 'silent', alias: aliasDirs,
    plugins: [{
      name: 'imports-only',
      setup(b) {
        ...
        b.onLoad({ filter: /\.m?js$/ }, async args => {
          ...
          while ((m = assetRe.exec(src))) {
            ...
            if (!toUrl(abs)) continue
            const stat = await fsp.stat(abs).catch(() => null)
            const files = stat?.isDirectory() ? await fsp.readdir(abs, {recursive: true}) : stat?.isFile() ? [''] : []
            for (const file of files) {
              const asset = path.join(abs, file)
              if ((await fsp.stat(asset)).isFile()) assets.add(path.relative(baseDir, asset))
              if (/\.m?js$/.test(asset)) importLines.push(`import '${asset}'`)
            }
          }
  ...
  const inputs = servedInputs([...new Set([...Object.keys(result.metafile.inputs), ...assets])], baseDir, path, toUrl)
    .filter(({ url }) => !url.startsWith(runtimeLibUrl))
  ...
  const bodies = await Promise.all(inputs.map(({ file }) => fsp.readFile(path.resolve(baseDir, file))))
  const uploadResult = await wputMany(inputs.map(({ file, url }, index) => ({
    url: `${clientCodeBase}${url.slice(1)}`,
    body: bodies[index],
  ...
  return { appletV, clientCodeWUrl, shareImports: imports, fileCount: inputs.length, totalBytes: bodies.reduce((sum, body) => sum + body.length, 0),
...
export async function uploadLambdaCompDependencies(entryPath) {
  ...
  const absEntry = onDisk(entryPath)
  ...
  const result = await esbuild.build({
    stdin: { contents: `import '${absEntry}'\nimport '@jb6/core/misc/jb-remote.js'`, resolveDir: baseDir }, absWorkingDir: baseDir,
      ...
      setup(b) {
        b.onResolve({ filter: /.*/ }, args => {
          const resolved = path.isAbsolute(args.path) ? args.path
            : args.path.startsWith('.') ? path.resolve(args.resolveDir, args.path) : onDisk(args.path)
  ...
  const inputs = servedInputs(Object.keys(result.metafile.inputs), baseDir, path, toUrl)
    .filter(({ url }) => !lambdaTraversalBlockers.test(url) && (testEntry || !lambdaTestEntry.test(url)))

  // Stage files into a tmp dir laid out by served url; the loader below maps every alias of the import map to its served dir.
  ...
  for (const { file, url } of inputs) {
    const target = path.join(stageRoot, url.slice(1))
    await fsp.mkdir(path.dirname(target), { recursive: true })
    await fsp.copyFile(path.resolve(baseDir, file), target)
  }
  timer.phase('stage')
...
const MAP = ${JSON.stringify(Object.fromEntries(Object.entries(imports).filter(([alias, url]) => alias.endsWith('/') && url.endsWith('/'))
    .map(([alias, url]) => [alias.slice(0, -1), runtimeBase + url.slice(0, -1)])), null, 2)}
...
`
  ...
  await fsp.writeFile(path.join(stageRoot, 'index.js'), `import './${toUrl(absEntry).slice(1)}'\n`)
  ...
  impl: mcpTool({
    ...
    text: async (ctx, {}, {compFullId, roomWUrl}) => {
      ...
      try {
        ...
        const lambdaId = comp.id, compPath = (await repoLayout()).toImportUrl(comp.$location.path) || comp.$location.path
...
Tool('uploadRoomApplet', {
  description: 'Publish a react comp and return a content-versioned shareUrl; signedRoom:// must be explicit.',
  params: [
    ...
    {id: 'ogImage', as: 'string', description: 'public JPEG/PNG URL: HTTP 200, image/*, 1200x630, under 600KB for WhatsApp.'},
    {id: 'ogImageLocalPath', as: 'string', description: 'local image with the same limits; overrides ogImage and gets a content-versioned URL.'}
  ],
  impl: mcpTool({
    vars: [testAdminUser()],
    text: async (ctx, {}, {roomId, entryCompFullId, ogTitle, ogDescription, ogImage, ogImageLocalPath}) => {
      ...
      const entryPath = (await repoLayout()).toImportUrl(comp.$location.path)
      if (!entryPath) return JSON.stringify({error: `${comp.$location.path} is not served by any package.json staticMappings entry`})
      const script = `
...
try {
  ...
  await coreUtils.writeServiceResult({ ...await uploadCompDependencies(${JSON.stringify(entryPath)}), cmpId })
    ...
    const shareKey = (ogTitle || ogDescription || ogImage || ogImageLocalPath) && (await import('crypto')).createHash('sha256')
      .update(JSON.stringify([ogTitle, ogDescription, ogImage])).update(imageBytes).digest('hex').slice(0, 12)
    const shareUrl = shareKey ? `${entryUrl}?share=${shareKey}` : entryUrl
    const imageName = ogImageLocalPath && `og-${shareKey}.${ogImageLocalPath.split('.').pop()}`
    const imageWUrl = imageName && `${roomWUrl}/applets/${cmpId}/${imageName}`
    const imageUrl = imageWUrl && (await wresolveInfo(imageWUrl, dbCtx, 'PUT')).resolved
    const og = Object.fromEntries(Object.entries({ogTitle, ogDescription, ogImage: imageUrl || ogImage, shareKey}).filter(([, v]) => v))
    ...
    return JSON.stringify({...result, imageUrl, defPath: `${roomWUrl}/applets/${cmpId}.json`, entryUrl, shareUrl})
  }
  })
})
...
const appletResults = await Promise.all(applets.map(async ({ name, url, def }) => {
  ...
  try {
    ...
    const { appletV, clientCodeWUrl, shareImports, fileCount } = uploadResult
    await save(url, { ...def, appletV, clientCodeWUrl, shareImports, roomWUrl })
