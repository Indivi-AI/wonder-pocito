...
The output is under infra/export/releases/RELEASE_VERSION: three images in images.tar, the chart, image IDs, and SHA256SUMS.
...
```sh
helm lint infra/prod/helm/pocito -f infra/prod/customer-values.example.yaml
helm template pocito infra/prod/helm/pocito -f infra/prod/customer-values.example.yaml
```

The TGP tests are registered through e2e-tests/main.js:
