...
COPY infra/dev/flapi-mock/package.json infra/dev/flapi-mock/package-lock.json /tmp/flapi-npm/
RUN --mount=type=secret,id=npmrc,target=/root/.npmrc \
...
COPY marketplace-server/pyproject.toml marketplace-server/uv.lock /opt/pocito/manifests/marketplace-server/
COPY agno-server/pyproject.toml agno-server/uv.lock /opt/pocito/manifests/agno-server/
COPY infra/dev/litellm/pyproject.toml infra/dev/litellm/uv.lock /opt/pocito/manifests/litellm/
...
COPY infra/dev/container/git-attributes /etc/pocito/gitattributes
...
COPY infra/dev/container/omp.mjs infra/dev/container/omp-minimax.mjs /opt/pocito/omp/
COPY --chmod=755 infra/dev/container/omp.sh /usr/local/bin/omp
COPY --chmod=755 infra/dev/container/entrypoint.sh /usr/local/bin/pocito-entrypoint
COPY --chmod=755 infra/dev/container/start-sshd.sh /usr/local/bin/start-pocito-sshd
COPY infra/dev/container/sshd_config /etc/ssh/sshd_config_pocito
