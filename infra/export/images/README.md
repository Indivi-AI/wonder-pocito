...
```sh
npm run export-dev
npm run export-dev -- --code
npm run export-dev -- --images
```
...
Every mode writes this standalone `README.md` into `infra/export/images/`, using the air-gapped section of the repository README.
...
cp "$POCITO_CHECKOUT/.env.example" "$POCITO_CONFIG/pocito.env"
cp "$POCITO_CHECKOUT/infra/dev/litellm/config.yaml" "$POCITO_CONFIG/litellm.yaml"
...
Agno writes working copies of skills, Python tools and knowledge documents from MinIO under `/home/pocito/.local/share/pocito/marketplace`; they can be recreated.
...
Run the installation suite at `http://LINUX_HOST:58000/studio/tests.html?pattern=pocitoOnPrem&includeHeavy`.
...
Export with `npm run export-dev -- --code` on the source machine. Replace the bundle, `SHA256SUMS.code` and README in `POCITO_KIT` on Linux.
...
The test runs `npm run local`, verifies pgvector SQL and clean restart persistence, and checks external database/Agno configuration.
...
docker run --rm --platform linux/amd64 --network pocito-postgres-test \
  ...
  "$POCITO_IMAGE" node --test e2e-tests/dev/postgres.test.mjs
