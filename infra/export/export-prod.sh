#!/usr/bin/env bash
# Build the three production images, package the Helm chart and write a self-contained release directory under infra/export/releases/VERSION.
set -euo pipefail
repoRoot=$(cd "$(dirname "$0")/../.." && pwd)
version=${1:?Usage: export-prod.sh VERSION [OUTPUT_DIRECTORY]}
releaseDir=${2:-$repoRoot/infra/export/releases/$version}
prodDir=$repoRoot/infra/prod
chartDir=$prodDir/helm/pocito
...
cp "$prodDir/"*.example.yaml "$prodDir/README.md" "$prodDir/deployment-reference.md" "$prodDir/deploy-oc.sh" "$releaseDir/"
(cd "$releaseDir" && shasum -a 256 images.tar images.txt pocito-*.tgz *.example.yaml README.md deployment-reference.md deploy-oc.sh > SHA256SUMS)
