...
ReactComp('parquetLongPaper', {
  impl: comp({
    hFunc: (ctx, {react: {h, imported, useEffect, useState}}) => () => {
      const [html, setHtml] = useState(''), [error, setError] = useState('')
      useEffect(() => {
        let active = true
        jb.wonderUtils.wfetch2('room://aTeam/usersRO/papers/parquet-long.md', {method: 'GET'}, ctx)
          ...
          .then(md => {
            ...
            parts[0].insertAdjacentHTML('beforebegin',
              '<img class="cover-image" src="https://storage.googleapis.com/indiviai-wonder/aTeam/usersRO/papers/parquet-og-3.png">')
            ...
            ;[...doc.querySelectorAll('h1')].find(heading => /^15\./.test(heading.textContent))?.insertAdjacentHTML('beforebegin',
              '<img class="part-end-image" src="https://storage.googleapis.com/indiviai-wonder/aTeam/usersRO/papers/parquet-og-4.png">')
            const tiers = [...doc.querySelectorAll('h1')].find(heading => /^4\. Scanning Implementation Tiers/.test(heading.textContent))
            if (tiers) {
              const section = doc.createElement('section')
              section.className = 'implementation-tiers'
              tiers.before(section)
              for (let node = tiers; node && (node === tiers || node.nodeName !== 'H1');) {
                const next = node.nextSibling
                section.append(node)
                node = next
              }
            }
            const benchmarks = [...doc.querySelectorAll('h1')].find(heading => /^Appendix B/.test(heading.textContent))
            if (benchmarks) {
              const section = doc.createElement('section')
              section.className = 'benchmark-appendix'
              benchmarks.before(section)
              for (let node = benchmarks; node;) {
                const next = node.nextSibling
                section.append(node)
                node = next
              }
      ...
      return h('main', {},
        h('style', {}, `:root{color-scheme:light;--ink:#17212b;--muted:#5d6873;--accent:#0d6b68;--line:#dce2df;--paper:#fff;--wash:#f3f6f4}
          *{box-sizing:border-box}body{margin:0;background:linear-gradient(135deg,#e8eeeb,#f7f4ed);color:var(--ink);font:16px/1.65 Georgia,serif}
          ...
          .implementation-tiers{width:calc(100vw - 32px);margin:3rem 0 3rem 50%;padding:32px clamp(24px,4vw,72px);
          transform:translateX(-50%);border-radius:12px;background:var(--paper);box-shadow:0 0 60px #263c3520}
          .benchmark-appendix{break-before:page;width:calc(100vw - 32px);margin:5rem 0 0 50%;padding:56px clamp(24px,4vw,72px);
          transform:translateX(-50%);border-top:14px solid #e8eeeb;border-radius:12px;background:var(--paper);box-shadow:0 0 60px #263c3520}
          .benchmark-appendix table,.implementation-tiers table{display:table;width:100%;overflow:visible;font-size:clamp(9px,.85vw,12px);line-height:1.3}
          .benchmark-appendix th,.benchmark-appendix td,.implementation-tiers th,.implementation-tiers td{padding:7px 9px}
    ...
    metadata: [
      ...
      applet({
        title: 'Why Parquet Structure Matters — Long Version',
        icon: 'BookOpen',
