...
Data('platformMarketplaceApiMinimax', {
  params: [
...
Data('platformAgnoRunMinimax', {
  ...
  impl: async ({}, {}, {message, agentId, sessionId, baseUrl, token}) => {
    ...
    Object.entries({message, session_id: sessionId, user_id: 'platform-v0-minimax', stream: 'false'})
  ...
  impl: comp({
    hFunc: (ctx, {react: {h, useEffect, useRef, useState}}) => () => {
      ...
      const serverUrl = ctx.vars.platformUrl || new URLSearchParams(location.search).get('platformUrl')
        || 'http://localhost:7777'
      const RESOURCES = {
        plugins: {icon: 'PlugZap', title: 'פלאגינים',
          subtitle: 'פלאגין אורז מיומנויות, כלים וסאב-אייג׳נטים ליחידה אחת.',
          createLabel: 'פלאגין חדש', badge: 'פלאגין'},
        skills: {icon: 'BookOpenText', title: 'מיומנויות',
          subtitle: 'הוראות וידע ממוקד שהסוכנים יכולים לטעון לפי הצורך.',
          createLabel: 'מיומנות חדשה'},
        tools: {icon: 'Wrench', title: 'כלים',
          subtitle: 'חיבורים ופעולות שהסוכנים יכולים להפעיל בזמן ריצה.',
          createLabel: 'כלי חדש'},
        reports: {icon: 'BadgeCheck', title: 'דוחות מאומתים',
          subtitle: 'שאילתות פרמטריות עם תצוגה עשירה ומוגדרת מראש.',
          createLabel: 'דוח מאומת חדש', verified: true},
        agents: {icon: 'Network', title: 'סאב-אייג׳נטים',
          subtitle: 'סוכנים ממוקדים שניתן להאציל אליהם משימות.',
          createLabel: 'סאב-אייג׳נט חדש'}
      }
      const LIBRARY_IDS = Object.keys(RESOURCES)
      const callApi = (method, resource, name, body) =>
        dsls.common.data.platformMarketplaceApiMinimax.$runWithCtx(ctx, {
          method, path: `/api/v1/${resource}${name ? `/${encodeURIComponent(name)}` : ''}`,
          body, baseUrl: serverUrl
        })
      const reloadMarketplace = async () => {
        const rows = await Promise.all(LIBRARY_IDS.map(async id => [id, await callApi('GET', id)]))
      ...
      const sendMessage = async () => {
        ...
        setAgentBusy(true)
        try {
          const run = await dsls.common.data.platformAgnoRunMinimax.$runWithCtx(ctx, {
            message, agentId: selectedAgent, sessionId: sessionId.current,
            baseUrl: serverUrl,
            token: ctx.vars.agnoToken
          })
          const parsed = await dsls.common.data.platformReportMarkersMinimax.$runWithCtx(ctx, {text: run.content})
          setMessages(xs => [...xs, {role: 'assistant', text: parsed.content, reports: parsed.reportIds, runId: run.runId}])
        } catch (error) {
      ...
      const switchView = id => (setView(id), setSearch(''))
      const navButton = (id, icon, title) => h(
        ...
        {key: id, onClick: () => switchView(id), 'aria-label': title}, h(`L:${icon}`, {size: 17}), title)
      const sidebar = h('aside:fixed top-0 right-0 bottom-0 z-40 hidden sm:flex w-[210px] flex-col border-l border-[#e4e8e5] bg-white p-4', {},
        h('div:flex items-center gap-3 px-2 pb-6 font-bold text-[#202724]', {},
          ...
          'פלאגין סטודיו'),
        navButton('plugins', RESOURCES.plugins.icon, RESOURCES.plugins.title),
        navButton('chat', 'MessageCircle', 'צ׳אט'),
        navButton('evaluation', 'SquareCheckBig', 'אבלואציה'),
        h('div:px-3 pb-2 pt-7 text-xs text-[#a4aaa7]', {}, 'ספרייה'),
        navButton('skills', RESOURCES.skills.icon, RESOURCES.skills.title),
        navButton('tools', RESOURCES.tools.icon, RESOURCES.tools.title),
        navButton('reports', RESOURCES.reports.icon, RESOURCES.reports.title),
        navButton('agents', RESOURCES.agents.icon, RESOURCES.agents.title))
      const mobileNav = h('nav:fixed bottom-0 left-0 right-0 z-50 flex border-t border-[#e4e8e5] bg-white sm:hidden', {},
        LIBRARY_IDS.map(id => {
          const r = RESOURCES[id]
          return h(`button:flex-1 py-2 text-[10px] ${view == id ? 'text-[#2f6b4b]' : 'text-gray-500'}`,
            {key: id, onClick: () => setView(id)},
            h(`L:${r.icon}`, {size: 18, className: 'mx-auto mb-0.5'}), r.title)
        }))
      const resourceCard = (item, resource) => {
        const r = RESOURCES[resource]
        return h('article:rounded-2xl border border-[#e4e8e5] bg-white p-5 shadow-[0_1px_2px_rgba(30,50,40,.04)]', {
          ...
          h('div:flex items-start gap-3', {},
            ...
            h('div:min-w-0 flex-1', {}, h('div:flex items-center gap-2', {},
              ...
              r.verified && h('span:flex items-center gap-1 text-[10px] font-semibold text-[#2f6b4b]', {},
                h('L:BadgeCheck', {size: 12}), 'מאומת')),
              h('div:mt-0.5 text-xs text-[#a1a7a4]', {}, r.badge || r.title)),
            h('button:rounded-lg p-1.5 text-[#a5aaa7] hover:bg-red-50 hover:text-red-600', {
              onClick: e => (e.stopPropagation(), deleteResource(resource, item.name)),
      ...
      const library = () => {
        const r = RESOURCES[view], items = (catalog?.[view] || [])
        ...
        return h('main:min-h-screen overflow-x-hidden px-5 pb-24 pt-8 sm:mr-[210px] sm:px-10 sm:pb-10', {},
          h('div:mx-auto max-w-6xl', {},
            h('div:flex flex-wrap items-start justify-between gap-4', {},
              h('div', {}, h('div:flex items-center gap-2', {}, h(`L:${r.icon}`, {size: 21, className: 'text-[#2f6b4b]'}),
                h('h1:text-2xl font-bold text-[#202724]', {}, r.title)), h('p:mt-2 text-sm text-[#929995]', {}, r.subtitle)),
              ...
              }, h('L:Plus', {size: 16}), r.createLabel)),
      ...
      const chat = () => h('main:min-h-screen overflow-x-hidden pb-28 sm:ml-[250px] sm:mr-[210px]', {},
        ...
        h('header:sticky top-0 z-20 flex items-center justify-between border-b border-[#e5e8e6] bg-white/95 px-6 py-4 backdrop-blur', {},
          h('div:flex items-center gap-3', {}, h('span:grid h-8 w-8 place-items-center rounded-xl bg-[#e6f2ea] text-xs font-bold text-[#285a40]', {}, 'אנ'),
            h('b:text-sm text-[#202724]', {}, catalog?.plugins?.find(x => x.name == selectedAgent)?.title || 'x')),
      ...
      const skillUploader = () => h('label:mt-4 flex cursor-pointer items-center justify-center gap-2 rounded-xl ' +
        ...
        h('input:hidden', {type: 'file', accept: '.md,.zip,.json,.yaml,.yml', onChange: async e => {
          ...
          setDraft(x => ({...x, title: x.title || baseName,
      ...
      const modal = draft && h('div:fixed inset-0 z-[70] grid place-items-center bg-black/25 p-4', {
        onMouseDown: e => e.target == e.currentTarget && setDraft(null)
          ...
          draft.resource == 'skills' && skillUploader(),
...
Data('platformReportMarkersMinimax', {
  params: [
  ...
  impl: async ({}, {}, {text}) => {
    const seen = new Set()
    const reportIds = []
    for (const match of text.matchAll(/\[\[report:([\w-]+)\]\]/g)) {
      const id = match[1]
      if (!seen.has(id)) { seen.add(id); reportIds.push(id) }
    }
    const content = text.replace(/\s*\[\[report:[\w-]+\]\]/g, '').trim()
    return {content, reportIds}
  }
})
