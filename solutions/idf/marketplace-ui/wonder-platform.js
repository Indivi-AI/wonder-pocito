...
ReactComp('wonderPlatform', {
  ...
  impl: comp({
    hFunc: (ctx, {react: {h, hh, useEffect, useState}},
      ...
      const blank = resource => ({id: '', name: '', desc: '', instructions: '',
      ...
      const openWorkspaceEditor = async (resource, item) => {
        if (!item) return
        if (repo.marketplace) item = await marketplaceDetail(ctx.setVars({resource, id: item.id, roomWUrl: repositoryRoomWUrl,
