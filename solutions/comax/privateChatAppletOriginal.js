...
const todayFocus = [
  ['מכירות היום', 'TrendingUp', 'מול יום דומה וסניפים מובילים',
    'בדוק את המכירות היום מול יום דומה: איפה אנחנו מעל או מתחת לקצב?'],
  ['מבצעים פעילים', 'BadgePercent', 'מה מוכר, מה תקוע ומה נשחק',
    'איך המבצעים הפעילים מתפקדים היום במכירות, רווחיות ומלאי?'],
  ['מכירות במבצע', 'ShoppingBasket', 'נתח פדיון שמגיע מהנחות',
    'כמה מהמכירות היום הגיעו ממבצעים, ובאילו קטגוריות זה עובד הכי טוב?'],
  ['רווחיות מבצעים', 'CircleDollarSign', 'מי משתלם ומי שורף מרווח',
    'אילו מבצעים שוחקים רווחיות ומה כדאי לשנות היום?'],
]
...
const cockpit = (h, send) =>
  h(
    'div:max-w-5xl mx-auto px-5 sm:px-6 pt-6 sm:pt-10 pb-10',
    {},
    h(
      'div:flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6',
      {},
      h(
        'div',
        {},
      ...
      h('div:flex items-center gap-2 rounded-full bg-[#61A60E] px-5 py-3 text-sm font-bold text-white shadow-sm', {},
        h('L:Sparkles', { size: 16 }), 'מוקד היום: מבצעים'),
    ),
    h(
      'div:grid grid-cols-2 lg:grid-cols-4 gap-3',
      {},
      todayFocus.map(([domain, icon, hint, q]) =>
        h(
          'button:text-right rounded-lg border border-gray-200 bg-white p-3 sm:p-4 shadow-sm hover:border-gray-300 hover:shadow-md transition-all',
          { key: domain, onClick: () => send(q) },
          h(
            'div:flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3',
            {},
            ...
            h(
              'span',
              {},
              h('span:block text-sm sm:text-base font-semibold text-[#454343]', {}, domain),
              h('span:block mt-1 text-[11px] sm:text-xs leading-5 text-gray-500', {}, hint),
            ),
          ),
    ...
    h(
      ...
      h(
        ...
        suggestedQueries.map((q) =>
          h(
            'button:rounded-full border border-[#61A60E]/30 bg-white px-3 py-2 text-sm font-semibold text-[#4a7f0b] hover:bg-[#61A60E]/10 transition-colors',
