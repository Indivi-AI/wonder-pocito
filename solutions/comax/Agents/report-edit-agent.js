...
const editDraft = async (ctx, args) => {
  const { userMessage = ctx.vars.userMessage, chatHistory = ctx.vars.chatHistory, currentReport = baseReport(ctx), currentPatch = ctx.vars.currentPatch, reportId = REPORT, roomWUrl = 'signedRoom://comaxDemo/usersRW', reportsRoot, selectedSlotKey = 'coverage.summary', saveDraft = true, model = MODEL } = args
  ...
  const draftUrl = roomFile(roomWUrl, id), draftWrite = saveDraft && validation.ok && await wfetch2(draftUrl, {
    method: 'PUT', body: JSON.stringify(savedPatch), headers: {'content-type': 'application/json'}}, ctx).catch(e => ({ error: e.message }))
...
Data('editReportStudioDraft', { params: [{ id: 'userMessage', as: 'string', mandatory: true }, { id: 'chatHistory' }, { id: 'currentReport' }, { id: 'currentPatch' }, { id: 'reportId', as: 'string', defaultValue: REPORT }, { id: 'roomWUrl', as: 'string', defaultValue: 'signedRoom://comaxDemo/usersRW' }, { id: 'reportsRoot', as: 'string' }, { id: 'selectedSlotKey', as: 'string', defaultValue: 'coverage.summary' }, { id: 'saveDraft', as: 'boolean', defaultValue: true }, { id: 'model', as: 'string', defaultValue: MODEL }], impl: (ctx, {}, args) => editDraft(ctx, args) })
Data('publishReportStudioDraft', { params: [{ id: 'report' }, { id: 'reportId', as: 'string', defaultValue: REPORT }, { id: 'roomWUrl', as: 'string', defaultValue: 'signedRoom://comaxDemo/usersRW' }], impl: async (ctx, {}, { report = baseReport(ctx), reportId, roomWUrl }) => ({ ok: (await wfetch2(roomFile(roomWUrl, reportId, ''), { method: 'PUT', body: JSON.stringify(report), headers: {'content-type': 'application/json'} }, ctx)).ok, cleared: (await wfetch2(roomFile(roomWUrl, reportId), { method: 'PUT', body: JSON.stringify({ reportId, ops: [], publishedAt: Date.now() }), headers: {'content-type': 'application/json'} }, ctx)).ok }) })
Data('reportStudioDraftValid', { moreTypes: 'boolean<common>', impl: ctx => !!(ctx.data?.report && ctx.data?.validation?.ok && !ctx.data?.error) })
FlowElem('editReportStudioDraft', { params: [{ id: 'userMessage', as: 'string', dynamic: true, defaultValue: '%$userMessage%' }, { id: 'chatHistory' }, { id: 'reportId', as: 'string', defaultValue: REPORT }, { id: 'roomWUrl', as: 'string', defaultValue: 'signedRoom://comaxDemo/usersRW' }, { id: 'reportsRoot', as: 'string' }, { id: 'selectedSlotKey', as: 'string', defaultValue: 'coverage.summary' }, { id: 'model', as: 'string', defaultValue: MODEL }], impl: async (ctx, {}, p) => ctx.setData(await editDraft(ctx, { ...p, userMessage: typeof p.userMessage == 'function' ? p.userMessage(ctx) : p.userMessage, currentReport: ctx.vars.currentReport })) })
