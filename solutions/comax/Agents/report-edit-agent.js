...
const editDraft = async (ctx, args) => {
  ...
  const draftUrl = roomFile(roomWUrl, id), draftWrite = saveDraft && validation.ok && await wfetch2(draftUrl, { method: 'PUT', body: savedPatch }, ctx).catch(e => ({ error: e.message }))
