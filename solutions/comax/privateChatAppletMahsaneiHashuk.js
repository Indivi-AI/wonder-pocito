...
const marketLogo = 'https://d226b0iufwcjmj.cloudfront.net/retailers/1107/logo.png'
const marketKpis = [
  ['פדיון עד 15:00', '₪3.18M', '76% מהיעד · +6.9%', 'TrendingUp', '#16a34a'],
  ['סל ממוצע', '₪124.6', '+₪5.3 מול שבוע שעבר', 'ShoppingCart', '#2563eb'],
  ['מלאי בסיכון', '27 פריטים', '9 במבצע פעיל', 'TriangleAlert', '#dc2626'],
  ['רווחיות מבצעים', '31.4%', '+1.8 נק׳ מול יעד', 'BadgePercent', '#7c3aed'],
]
const marketQuestions = [
  ['למה אשדוד נחלשת?', 'השווה שעות, קטגוריות ומבצעים מול סניפים דומים',
    'למה סניף אשדוד מתחת ליעד היום?'],
  ['מה להבליט עכשיו?', 'מצא מבצע עם ביקוש גבוה, מלאי טוב ושחיקת רווח נמוכה',
    'אילו מבצעים כדאי להבליט בשעות הקרובות?'],
  ['מה חסר במדף?', 'אתר פריטי ביקוש עם פחות מיומיים מלאי', 'אילו פריטי מבצע בסיכון חוסר מלאי היום?'],
]
const trend = [
    ['09', 31],
    ['10', 42],
  ...
  trendPath = trend.map(([, v], i) => `${i ? 'L' : 'M'}${14 + i * 48} ${106 - v}`).join(' ')
const branchBars = [
  ['באר שבע גרנד', 106, '+14%'],
  ['נתניה עיר ימים', 99, '+6%'],
  ['חיפה נווה שאנן', 94, '+2%'],
...
const cockpit = (h, send) =>
  h(
    'div:max-w-5xl mx-auto px-5 sm:px-6 pt-0 sm:pt-2 pb-44',
    {},
    h(
      'div:rounded-lg bg-white border border-gray-200 p-4 sm:p-6 shadow-xl overflow-hidden',
      {},
      h(
        'div:flex flex-col lg:flex-row gap-6 lg:items-start lg:justify-between',
        {},
        h(
          'div:max-w-2xl',
          {},
          h(
            'div:inline-flex items-center gap-3 rounded-lg bg-[#F7FAF3] border border-[#61A60E]/20 px-4 py-2',
            {},
            h('img:h-10 max-w-40 object-contain', { src: marketLogo, alt: 'מחסני השוק' }),
            h('span:text-xs font-bold text-[#4a7f0b]', {}, 'Retail cockpit'),
          ),
          h('h1:mt-5 text-3xl sm:text-5xl font-bold leading-tight text-[#17351f]', {}, 'בוקר טוב מחסני השוק'),
          h('p:mt-3 text-sm sm:text-base leading-7 text-gray-600', {},
            'תמונת מצב עדכנית מהרשת: קצב מכירות, סל ממוצע, מלאי בסיכון ורווחיות מבצעים מתוך Comax.'),
        ),
        h(
          'div:w-full lg:max-w-sm rounded-lg border border-gray-200 bg-[#F8FAFC] p-4',
          {},
          h(
            'div:flex items-center justify-between text-sm',
            {},
            h('span:font-bold text-gray-900', {}, 'קצב מול יעד יומי'),
            h('span:rounded-full bg-[#61A60E] px-2 py-1 text-xs font-bold text-white', {}, 'חי'),
          ),
          h(
            'svg:w-full h-32 mt-4 overflow-visible',
            { viewBox: '0 0 320 120', role: 'img', 'aria-label': 'מגמת פדיון עולה' },
            h('path', { d: 'M14 106H306', stroke: '#E5E7EB' }),
            h('path', { d: trendPath, stroke: '#61A60E', strokeWidth: 4, fill: 'none', strokeLinecap: 'round', strokeLinejoin: 'round' }),
            trend.map(([t, v], i) => h('circle', { key: t, cx: 14 + i * 48, cy: 106 - v, r: 4, fill: '#17351f' })),
          ),
          h(
            'div:grid grid-cols-7 gap-1 text-center text-[10px] text-gray-500',
            { dir: 'ltr' },
            trend.map(([t]) => h('span', { key: t }, t)),
          ),
        ),
      ),
      h(
        'div:grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-5',
        {},
        marketKpis.map(([label, value, hint, icon, color]) =>
          h(
            'div:rounded-lg bg-[#F8FAFC] border border-gray-200 p-4 text-[#17351f]',
            { key: label },
            h('div:flex items-center justify-between gap-3', {}, h('span:text-sm font-bold', {}, label), h(`L:${icon}`, { size: 18, color })),
            h('div:mt-3 text-2xl font-bold', {}, value),
            h('div:mt-1 text-xs text-gray-500', {}, hint),
          ),
        ),
      ),
      h(
        'div:grid grid-cols-1 lg:grid-cols-2 gap-4 mt-5',
        {},
        h(
          'div:rounded-lg bg-[#F8FAFC] border border-gray-200 p-4',
          {},
          h('div:text-sm font-bold text-gray-900 mb-3', {}, 'סניפים מול יעד יומי'),
          branchBars.map(([name, value, delta]) =>
            h(
              'div:flex items-center gap-2 text-xs mb-2 last:mb-0 text-gray-700',
              { key: name },
              h('span:w-24 shrink-0 truncate', {}, name),
              h('div:flex-1 h-2 rounded-full bg-gray-200 overflow-hidden', {}, h('div:h-full rounded-full bg-[#61A60E]', { style: { width: `${Math.min(value, 100)}%` } })),
              h('span:w-20 text-left font-bold whitespace-nowrap', { dir: 'ltr', style: { color: delta[0] == '-' ? '#dc2626' : '#16a34a' } }, `${value}% ${delta}`),
            ),
          ),
        ),
        h(
          ...
          {},
          marketQuestions.map(([title, hint, q]) =>
            h(
              'button:text-right rounded-lg bg-white border border-gray-200 px-4 py-3 text-[#17351f] shadow-sm hover:bg-[#F7FAF3] transition-colors',
              { key: title, onClick: () => send(q) },
              h('div:flex items-center justify-between gap-3', {}, h('span:font-bold', {}, title), h('L:ArrowLeft', { size: 15, color: '#16a34a' })),
              h('div:mt-1 text-xs leading-5 text-gray-500', {}, hint),
            ),
          ),
        ),
      ),
...
const mdInline = (h, s) =>
  String(s)
    ...
    .map((p, i) =>
      p.startsWith('**')
        ? h('strong:font-semibold text-gray-900', { key: i }, p.slice(2, -2))
        : p.startsWith('`')
          ? h('code:px-1 py-0.5 rounded bg-gray-100 text-[0.85em] font-mono', { key: i }, p.slice(1, -1))
          : p.startsWith('*')
            ? h('em', { key: i }, p.slice(1, -1))
            : p,
    )
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (head) {
      const lv = head[1].length,
        sz = ['text-xl', 'text-lg', 'text-base', 'text-sm'][lv - 1]
      out.push(h(`h${lv}:${sz} font-semibold text-gray-900 mt-4 first:mt-0 mb-1`, { key: out.length }, mdInline(h, head[2])))
      continue
    }
    const ordered = /^\s*\d+\.\s+/.test(ln),
      item = ordered ? /^\s*\d+\.\s+/ : /^\s*[-*]\s+/
    if (ordered || item.test(ln)) {
      const items = []
      while (i < lines.length && item.test(lines[i])) items.push(h('li:leading-relaxed', { key: items.length }, mdInline(h, lines[i++].replace(/^\s*(?:\d+\.|[-*])\s+/, ''))))
      i--
      out.push(h(`${ordered ? 'ol:list-decimal' : 'ul:list-disc'}:pr-5 my-1 space-y-0.5 marker:text-gray-400`, { key: out.length }, items))
      continue
    }
...
function renderMessageContent(element, ctx) {
  ...
  if (type === 'image' || (typeof content === 'string' && (/^data:image\//i.test(content) || /\.(png|jpe?g|gif|webp)(\\?.*)?$/i.test(content)))) {
    ...
    const nodes = h(
      ...
      h('a', { href: url, target: '_blank', rel: 'noopener' }, h('img:max-w-80 rounded-lg', { src: url })),
    )
  ...
  if (type === 'poll') {
    const nodes = h(
      ...
      ...(element.pollOptions || []).map((opt, i) =>
        h('div:bg-gray-100 rounded-full px-3 py-1.5 mb-1 text-sm flex items-center', { key: i }, h('div:w-4 h-4 rounded-full border-2 border-gray-400 ml-2'), opt),
      ),
    )
    return { nodes, hostContent: null }
  }
    ...
    hostContent = h(
      ...
      element.host
        .map((hostElement, i) => {
          ...
          if (hostElement.type === 'applet-link')
            return h(
              'button:text-xs text-blue-500 underline p-1',
              {
                key: i,
...
const rowVal = (ctx, rows, exp) => {
  try {
    return fmtRowVal(coreUtils.compileJb(exp.trim())(ctx.setVars({ rows })).next().value)
  } catch {
    return null
  }
}
const fillRows = (ctx, rows, s) =>
  !Array.isArray(rows) || typeof s != 'string'
    ? s
    : s
        .replace(/\$\(\(([\s\S]*?)\)\)/g, (m, exp) => {
          const v = rowVal(ctx, rows, exp)
          return v == null ? m : `$${v}`
        })
        .replace(/\(\(([\s\S]*?\$rows[\s\S]*?)\)\)/g, (m, exp) => rowVal(ctx, rows, exp) ?? m)
        .replace(/\((\$rows\[\d+\]\.[a-zA-Z_]\w*)\)/g, (m, exp) => rowVal(ctx, rows, exp) ?? m)
const fillObjRows = (ctx, rows, v) =>
  typeof v == 'string'
    ? fillRows(ctx, rows, v)
    : Array.isArray(v)
      ? v.map((x) => fillObjRows(ctx, rows, x))
      : v && typeof v == 'object'
        ? Object.fromEntries(Object.entries(v).map(([k, x]) => [k, fillObjRows(ctx, rows, x)]))
        : v
...
const AnalyticsAssistantResponse = ReactComp('AnalyticsAssistantResponse', {
  impl: comp({
    hFunc:
      (ctx, { isDebug, colors = { text: '#111827' }, react: { h, hh, useState } }) =>
      ({ element, send, openDetails }) => {
        ...
        const widgets = (Array.isArray(element?.widgets) ? element.widgets : [])
          .slice().sort((a, b) => (a?.kind == 'table') - (b?.kind == 'table')) // charts first, tables after
        ...
        const openDrill = async (spec, e) => {
          const d = coreUtils.asArray(spec?.drill).find((x) => x?.sql || x?.question),
            fill = (s) => s && jb.vizUtils.fillDrill(s, e),
            title = fill(d?.title) || e?.name || spec?.title || 'פירוט'
          if (d?.sql) {
            openDetails({ title, loading: true, question: fill(d.question) })
            try {
              const rows = await drillSqlRunner(ctx)(fill(d.sql)),
                kind = d.kind || spec.kind || 'table'
              ...
              openDetails({ title, error: String(err?.message || err) })
            }
          } else {
        ...
        return h(
          'div:w-full max-w-3xl mx-auto px-5 sm:px-6 py-3',
          {},
          narrative && h('div:mb-4 text-sm text-gray-500 leading-6', {}, narrative),
          collapsedParts
            ? h(
                'div:space-y-3 text-[16px] leading-7 text-gray-900',
                { style: { color: colors.text, overflowWrap: 'anywhere' } },
                h('div', {}, nodes),
                parts.map((p, i) =>
                  h(
                    'details:rounded-lg border border-gray-200 bg-white px-3 py-2',
                    { key: i },
            ...
            : parts.length
              ? h(
                  'div:text-[16px] leading-7 text-gray-900',
                  { style: { color: colors.text, overflowWrap: 'anywhere' } },
                  parts.map((p, i) => {
                    ...
                    return h(
                      ...
                      q &&
                        depth > 0 &&
                        h(
                          'button:mt-1 text-xs text-gray-600 hover:text-gray-900 inline-flex items-center gap-1',
                          { onClick: () => send?.(q) },
                          h('L:Search', { size: 13 }),
                          p.drill?.label || 'נתח את זה',
                        ),
                    )
                  }),
                )
          ...
          parts.length > 0 &&
            !collapsedParts &&
            h(
              'div:mt-3 flex items-center gap-3 text-xs text-gray-500',
              {},
              h('input:flex-1 min-w-0 accent-[#61A60E]', {
                type: 'range', min: 0, max: 2, value: depth, onInput: (e) => setDepth(+e.target.value), 'aria-label': 'אורך תשובה',
              }),
              h(
                'button:px-2.5 py-1.5 rounded-full border border-gray-200 bg-white text-gray-700 disabled:text-gray-400 inline-flex items-center gap-1',
                { disabled: depth == 2, onClick: () => setDepth(Math.min(2, depth + 1)) },
                h('L:ChevronDown', { size: 13 }),
                depth == 2 ? 'עמוק' : 'העמק',
              ),
            ),
          ...
          !reactComps.length &&
            widgets.length > 0 &&
            h(
              'div:mt-5 space-y-5 overflow-x-hidden',
              {},
              widgets.map((spec, i) =>
                h(
                  'div:min-w-0 overflow-x-auto',
                  { key: i },
                  hh(ctx, VizWidget, {
                    spec: { width: 680, height: 340, ...(openDetails ? drawerSpec(spec) : spec) },
                    runSql: drillSqlRunner(ctx),
                    onEvent: openDetails
                      ? (e) => openDrill(spec, e)
                      : send &&
                        ((e) => {
          ...
          followUps.length > 0 &&
            h(
              ...
              followUps.map((f, i) =>
                h(
                  'button:px-3 py-1.5 text-sm rounded-full border border-[#61A60E]/40 bg-white text-[#4a7f0b] hover:bg-[#61A60E]/10 transition-colors',
                  {
                    key: i,
          ...
          (sql || (Array.isArray(rows) && rows.length)) &&
            h(
              'div:mt-3',
              {},
              h(
                'button:text-xs text-gray-500 hover:text-gray-700 inline-flex items-center gap-1',
                { onClick: () => setShowData((v) => !v) },
                h('L:Database', { size: 12 }),
                showData ? 'הסתר SQL / נתונים' : 'הצג SQL / נתונים',
              ),
              showData &&
                h(
                  'div:mt-2 space-y-2',
          ...
          showAdminLink &&
            h(
              ...
              {},
              h(
                'a:text-xs text-blue-500 hover:text-blue-700 underline',
                { href: adminUrl, target: '_blank', rel: 'noopener' },
...
const AnalyticsComposer = ReactComp('AnalyticsComposer', {
  impl: comp({
    hFunc:
      (ctx, { react: { h, useEffect, useRef, useState } }) =>
      ({ send, disabled, selected, setSelected, multi, model = '', setModel = () => {} }) => {
        const [txt, setTxt] = useState(''),
          [open, setOpen] = useState(false),
          ref = useRef(null),
          canSend = !!txt.trim() && !disabled
        ...
        const toggle = (id) => (multi ? setSelected((s) => (s.includes(id) ? (s.length > 1 ? s.filter((x) => x != id) : s) : [...s, id])) : (setSelected([id]), setOpen(false)))
        const label = (selected || []).map((id) => byId[id]?.label || id).join(' + ') || 'סוכן'
        return h(
          'div:rounded-[22px] border border-gray-200 bg-white shadow-[0_10px_35px_rgba(0,0,0,0.09)] overflow-visible',
          {},
          h(
            ...
            h('textarea:flex-1 min-h-[48px] max-h-44 resize-none bg-transparent px-3 py-3 text-[15px] leading-6 text-gray-900 placeholder:text-gray-400 focus:outline-none', {
              ...
              dir: 'rtl',
              rows: 1,
              placeholder: 'שאלו על נתוני Comax ERP...',
              onInput: (e) => setTxt(e.target.value),
              onKeyDown: (e) => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), submit()),
            }),
            h(
              'button:w-10 h-10 mb-1 flex items-center justify-center rounded-full bg-[#61A60E] text-white disabled:bg-gray-200 disabled:text-gray-400 transition-colors',
              { disabled: !canSend, onClick: submit, 'aria-label': 'שליחה' },
              h('L:ArrowUp', { size: 17, strokeWidth: 3 }),
            ),
          ),
          h(
            'div:relative px-3 pb-2 flex flex-wrap items-center gap-2',
            {},
            open && h('div:fixed inset-0 z-40', { onClick: () => setOpen(false) }),
            multi &&
              h(
                'select:px-2.5 py-1.5 rounded-full border border-gray-200 bg-white text-xs font-medium text-gray-700 max-w-[9.5rem]',
                {
                  value: model,
            ...
            h(
              'button:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border border-gray-200 ' +
                'bg-white text-gray-700 hover:border-gray-300 transition-colors',
              {
                onClick: () => setOpen((v) => !v),
            ...
            open &&
              h(
                'div:absolute bottom-full right-3 mb-2 w-72 rounded-2xl border border-gray-200 bg-white shadow-xl p-1.5 z-50',
                {},
                agents.map((a) => {
                  const on = (selected || []).includes(a.id)
                  return h(
                    `button:w-full text-right px-3 py-2 rounded-xl flex items-start gap-2 transition-colors ${on ? 'bg-[#61A60E]/10' : 'hover:bg-gray-50'}`,
                    { key: a.id, onClick: () => toggle(a.id) },
                    multi
                      ? h(
                          `span:mt-0.5 w-4 h-4 rounded border flex items-center justify-center shrink-0 ${on ? 'bg-[#61A60E] border-[#61A60E] text-white' : 'border-gray-300'}`,
                          {},
                          on && h('L:Check', { size: 11, strokeWidth: 3 }),
                        )
                      : h(
                          `span:mt-0.5 w-4 h-4 rounded-full border flex items-center justify-center shrink-0 ${on ? 'border-[#61A60E]' : 'border-gray-300'}`,
                          {},
                          on && h('span:w-2 h-2 rounded-full bg-[#61A60E]'),
                        ),
                    h('span:flex-1', {}, h('span:block text-sm font-medium text-gray-800', {}, a.label), h('span:block text-xs text-gray-500', {}, a.hint)),
                  )
                }),
          ...
          h(
            'div:flex items-center justify-between gap-3 border-t border-gray-100 px-4 py-2 text-[11px] text-gray-400',
            {},
...
const detailLabel = {
  stock: 'מלאי', weekly_sales_avg: 'מכירות שבועיות', current_price: 'מחיר', last_sale_date: 'תאריך אחרון', receipts: 'קבלות',
}
const feedbackDetails = (o) =>
  o?.summary ||
  Object.entries(o?.details || {})
    .map(([k, v]) => `${detailLabel[k] || k}: ${fmtRowVal(v)}`)
    .join(' · ')
const feedbackNoun = (e) =>
  ({ product: 'products', products: 'products', branch: 'branches', branches: 'branches' })[
    String(e.entity || e.varName || '')
      .replace(/^selected/i, '')
      .toLowerCase()
  ] || 'options'
...
const feedbackItems = (e, ids) => {
  const opts = Object.fromEntries((e.options || []).map((o) => [String(o.id), o])),
    items = coreUtils.asArray(e.items || e.value?.items),
    labels = coreUtils.asArray(e.labels || e.value?.labels)
  return (ids.length ? ids : items.length ? items.map((x) => x.id) : labels)
    .map((id, i) => opts[String(id)] || items.find((x) => String(x.id) == String(id)) || { id, label: labels[i] || id })
    .filter(Boolean)
}
...
const viewedValues = (a) =>
  coreUtils
    .asArray(a)
    .slice(0, 30)
    .map((x) => (x?.label != null || x?.name != null || x?.value != null ? [x.label ?? x.name, x.value] : x?.x != null || x?.y != null ? [x.x, x.y] : x))
const viewedBlock = (w) => {
  const kind =
    { bar: 'InlineBarPlot', hbar: 'InlineBarPlot', pie: 'InlinePiePlot', line: 'InlineLinePlot', area: 'InlineLinePlot', table: 'InlineTable', kpi: 'InlineKpis' }[w?.kind] ||
    'InlineWidget'
  const body = w?.data
    ? ['values', viewedValues(w.data)]
    : w?.rows
      ? ['rows', coreUtils.asArray(w.rows).slice(0, 30)]
      : w?.items
        ? ['values', viewedValues(w.items)]
        : w?.series
          ? ['series', w.series]
          : []
...
const chatHistoryEntry = (m) => {
  ...
  return c
    ? { role: 'user', content: `Clarified ${c.varName}: ${c.labels.join(', ')} (ids: ${c.ids.join(',')})`, clarifiedParam: c }
    : {
        role: m.sender === 'Assistant' ? 'assistant' : 'user',
        content: m.content,
        ...(vd && { viewedData: vd }),
        ...(m.sql && { sql: m.sql }),
        ...(m.llmGeneratedCode && { llmFlow: m.llmGeneratedCode }),
      }
}
const HumanFeedbackMessage = ReactComp('HumanFeedbackMessage', {
  impl: comp({
    hFunc:
      ...
      ({ element, resolve }) => {
        const [ids, setIds] = useState(feedbackIds(element)),
          [open, setOpen] = useState(false),
          multi = element.mode == 'multi',
          done = element.resolved
        const has = (xs, id) => xs.some((x) => String(x) == String(id)),
          toggle = (id) => !done && setIds((xs) => (multi ? (has(xs, id) ? xs.filter((x) => String(x) != String(id)) : [...xs, id]) : [id]))
        const selected = feedbackItems(element, ids)
        return h(
          'div:w-full max-w-3xl mx-auto px-5 sm:px-6 py-3',
          {},
          h(
            'div:bg-white border border-gray-200 rounded-2xl p-4 shadow-sm',
            {},
            h('div:flex items-center gap-2 mb-3 text-sm font-semibold text-gray-900', {}, h('L:ListChecks', { size: 16 }), element.question),
              ...
              ? h(
                  ...
                  selected.length > 0 &&
                    h(
                      ...
                      selected.map((o) =>
                        h(
                          'span:inline-flex items-center gap-1 rounded-full bg-[#61A60E]/10 px-2.5 py-1 text-xs font-semibold text-[#4a7f0b]',
                          { key: o.id },
                          h('L:Check', { size: 12 }),
                          o.label || o.name || o.id,
                        ),
                  ...
                  h(
                    'button:inline-flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700',
                    { onClick: () => setOpen((x) => !x), 'aria-expanded': open },
                    h('L:ChevronDown', { size: 13, className: `transition-transform ${open ? 'rotate-180' : ''}` }),
                    `${element.autoResolved ? 'נבחר אוטומטית · ' : ''}selected ${selected.length} ${feedbackNoun(element)}`,
                  ),
                  open &&
                    h(
                      'div:space-y-1',
                      ...
                      selected.map((o) =>
                        h(
                          'div:rounded-lg bg-gray-50 px-3 py-2 text-sm text-gray-700',
                          { key: o.id },
              ...
              : h(
                  ...
                  (element.options || []).map((o) => {
                    ...
                    return h(
                      `button:w-full text-right rounded-xl border p-3 transition-colors ${on ? 'border-[#61A60E] bg-[#61A60E]/10' : 'border-gray-200 hover:bg-gray-50'}`,
                      { key: o.id, onClick: () => toggle(o.id) },
                      h(
                        'div:flex items-start gap-3',
                        {},
                        h(
                          `span:mt-0.5 w-5 h-5 rounded ${multi ? 'border' : 'rounded-full border'} flex items-center justify-center shrink-0 ` +
            ...
            !done &&
              h(
                'button:mt-3 px-4 py-2 rounded-full bg-[#61A60E] text-white text-sm font-semibold disabled:bg-gray-200 disabled:text-gray-400',
                { disabled: !ids.length, onClick: () => resolve(ids) },
                'המשך',
              ),
          ),
        )
  ...
  impl: comp({
    hFunc:
      ...
      ({ element, send, openDetails }) => {
        ...
        return h(
          'div:w-full max-w-3xl mx-auto px-5 sm:px-6 py-3 space-y-3',
          {},
          h('div:text-xs text-gray-500', {}, `השוואת ${items.length} סוכנים — הרצה במקביל`),
          items.map((it) => {
            const isOpen = !closed[it.agent]
            return h(
              'div:rounded-2xl border border-gray-200 bg-white overflow-hidden',
              { key: it.agent },
              h(
                'button:w-full flex items-center justify-between gap-2 px-4 py-2.5 hover:bg-gray-50 transition-colors',
                { onClick: () => setClosed((c) => ({ ...c, [it.agent]: isOpen })) },
                h(
                  'span:flex items-center gap-2 text-sm font-semibold text-gray-800',
                  {},
                  h('L:Bot', { size: 14 }),
                  byId[it.agent]?.label || it.agent,
                  it.error && h('span:text-xs font-medium text-red-600 bg-red-50 rounded-full px-2 py-0.5', {}, 'שגיאה'),
                ),
                h(
                  'span:flex items-center gap-2 text-xs text-gray-500',
                  {},
...
ReactComp('comaxDemoMahsaneiHashukApplet', {
  impl: comp({
    hFunc:
      (ctx, { roomId, isDebug, colors = { text: '#111827' }, wonderMain, initChatElements, shareShortUrl, react: { h, hh, useRef, useState, useEffect } }) =>
      () => {
        ...
        const [details, setDetails] = useState(null),
          [detailsW, setDetailsW] = useState(430)
        const typedStatusRef = useRef('')
        const typingTimerRef = useRef(0)
        const vw = globalThis.innerWidth || 1200,
          maxDetailsW = vw < 640 ? vw : Math.max(260, Math.min(720, vw - 64)),
          drawerW = Math.min(detailsW, maxDetailsW)
        const shift = details ? drawerW : 0
        const mainHidden = details && vw < 640,
          fixedStyle = mainHidden ? { display: 'none' } : { left: shift, transition: 'left .3s ease' }
        ...
        useEffect(() => {
          typedStatusRef.current = typedStatus
        }, [typedStatus])
        useEffect(() => {
        ...
        const newChat = () => {
          setHashParam('instanceId', formatTimeWithRandom())
          location.reload()
        }
        ...
        return h(
          'div:comax-brand min-h-screen bg-[#F6F7F8] text-gray-900 overflow-x-hidden',
          { dir: 'rtl', lang: 'he' },
          h('style', {}, comaxFontCss),
          details &&
            h(
              'aside:fixed top-0 bottom-0 left-0 z-40 bg-white border-r border-gray-200 shadow-xl flex flex-col',
              { style: { width: drawerW, paddingTop: 68 } },
              h(
                'div:flex items-center justify-between gap-3 px-4 py-3 border-b border-gray-100',
                {},
                h('div:min-w-0 text-sm font-bold text-gray-900 truncate', {}, details.title || 'פירוט'),
                h(
                  'button:w-8 h-8 rounded-full hover:bg-gray-100 flex items-center justify-center',
                  { onClick: () => setDetails(null), 'aria-label': 'סגור פירוט' },
                  h('L:X', { size: 16 }),
                ),
              ),
              h(
                'div:flex-1 min-h-0 overflow-auto p-4 space-y-3',
                {},
                ...
                details.content,
                details.childSpec &&
                  hh(ctx, dsls.react['react-comp'].VizWidget, { spec: { width: Math.max(260, drawerW - 32), height: 300, ...details.childSpec }, runSql: drillSqlRunner(ctx) }),
                details.question &&
                  h(
                    'button:inline-flex items-center gap-2 rounded-full bg-[#61A60E] px-4 py-2 text-sm font-bold text-white',
                    { onClick: () => send(details.question) },
          ...
          details &&
            h('div:fixed top-0 bottom-0 z-50 w-2 cursor-col-resize touch-none bg-transparent hover:bg-[#61A60E]/20 transition-colors', {
              style: { left: drawerW - 4 },
              onPointerDown: dragDetails,
            }),
          h(
            'nav:fixed top-0 left-0 right-0 z-50 px-4 py-3 bg-white/95 backdrop-blur border-b-2 border-[#61A60E]',
            { style: fixedStyle },
            h(
              'div:mx-auto max-w-5xl flex items-center justify-between',
              {},
              h(
                'button:w-10 h-10 flex items-center justify-center rounded-full hover:bg-gray-100',
                {
              ...
              h(
                ...
                h(
                  'button:w-10 h-10 flex items-center justify-center rounded-full hover:bg-gray-100',
                  { onClick: newChat, title: 'צ׳אט חדש', 'aria-label': 'צ׳אט חדש' },
                  h('L:SquarePen', { size: 18 }),
                ),
          ...
          h(
            'div:pt-20 pb-36 overflow-y-auto overflow-x-hidden',
            {
              ref: listRef,
              style: {
                height: '100vh',
                marginLeft: shift,
                width: `calc(100vw - ${shift}px)`,
                transition: 'margin-left .3s ease,width .3s ease',
                ...(mainHidden && { display: 'none' }),
              },
            },
            h(
              'div:mx-auto max-w-5xl py-4',
              {},
              ...
              visibleMessages.map((e) =>
                e.sender === 'Assistant'
                  ? h(
                      'div',
                      { key: e.id, id: `msg-${e.id}` },
                      hh(ctx, e.type === 'humanFeedback' ? HumanFeedbackMessage : e.type === 'agentComparison' ? AgentComparison : AnalyticsAssistantResponse, {
                        element: e,
                        send,
                        openDetails,
                        resolve: (ids) => resolveHumanFeedback(e, ids),
                      }),
                    )
                  : (() => {
                      const { nodes } = renderMessageContent(e, ctx)
                      return h(
                        'div:flex justify-start px-5 sm:px-6 my-5',
                        { key: e.id, id: `msg-${e.id}` },
                        h(
                          'div:flex flex-col items-start gap-1',
                          {},
                          h(
                            'div:max-w-[min(28rem,82vw)] px-4 py-2.5 rounded-2xl bg-[#EBF4DE] text-[15px] leading-6',
              ...
              streamingState.isTyping &&
                h(
                  'div:max-w-3xl mx-auto px-5 sm:px-6 py-4',
                  {},
                  h(
                    'div:flex items-center gap-3 text-gray-500',
                    {},
                    h(
                      'div:w-8 h-8 rounded-full bg-white border border-gray-200 text-[#61A60E] flex items-center justify-center',
                      {},
          ...
          newTextId &&
            !mainHidden &&
            h(
              'button:fixed left-1/2 -translate-x-1/2 bottom-28 sm:bottom-24 z-50 max-w-[calc(100vw-2rem)] inline-flex items-center gap-2 ' +
                'rounded-full bg-[#61A60E] px-4 py-2 text-sm font-bold text-white shadow-lg animate-pulse',
              {
              ...
              h('L:ArrowDown', { size: 16 }),
              'התשובה מוכנה',
            ),
          h(
            'div:fixed bottom-0 left-0 right-0 z-40 bg-[#F6F7F8]/85 backdrop-blur',
            { style: { ...fixedStyle, paddingBottom: 'env(safe-area-inset-bottom)' } },
            h(
              'div:mx-auto max-w-3xl px-4 py-3',
              {},
              hh(ctx, AnalyticsComposer, {
                send,
                disabled: streamingState.isTyping,
                selected,
                setSelected,
                multi: !!isDebug,
                model,
    ...
    enrichCtx: async (ctx, { roomId }) => {
      ...
      return roomCtx.setVars({
        initChatElements: await wGet(cts.privateChat, roomCtx),
        shareShortUrl,
        analyticsWorkflow: p.get('workflow'),
        comaxDataset: 'big',
        analyticsModel: p.get('model'),
        isDebug: ctx.vars.isDebug || p.get('debug') != null,
      })
    },
    metadata: applet({ title: 'Comax Demo - Mahsanei Hashuk', icon: 'BarChart3', showMessageInput: false }),
  }),
})
...
async function sendMessage({ txt, agents: agentIds, model, chatElements, setChatElements, ctx, setStreamingState, setNewTextId = () => {} }) {
  ...
  typeof requestAnimationFrame == 'function' && requestAnimationFrame(() => document.getElementById(`msg-${msg.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' }))
  await wAppend(cts.privateChat, msg, ctx)
  const upsertAssistant = (m) =>
    setChatElements((xs) => {
  ...
  const ctxForWf = ctx.setVars({
    chatElements: nextMsgs,
    setChatElements,
    userMessage: txt,
    db: 'local',
    comaxDataset: 'big',
    accumulatedContext: { chatHistory, clarifiedParams },
  })
