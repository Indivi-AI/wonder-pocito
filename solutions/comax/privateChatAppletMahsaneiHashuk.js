...
const marketQuestions = [
  ['למה אשדוד נחלשת?', 'השווה שעות, קטגוריות ומבצעים מול סניפים דומים', 'למה סניף אשדוד מתחת ליעד היום?'],
  ['מה להבליט עכשיו?', 'מצא מבצע עם ביקוש גבוה, מלאי טוב ושחיקת רווח נמוכה', 'אילו מבצעים כדאי להבליט בשעות הקרובות?'],
  ['מה חסר במדף?', 'אתר פריטי ביקוש עם פחות מיומיים מלאי', 'אילו פריטי מבצע בסיכון חוסר מלאי היום?']
]
const trend = [['09', 31], ['10', 42], ['11', 39], ['12', 51], ['13', 63], ['14', 71], ['15', 76]], trendPath = trend.map(([,v], i) => `${i ? 'L' : 'M'}${14 + i * 48} ${106 - v}`).join(' ')
const branchBars = [['באר שבע גרנד', 106, '+14%'], ['נתניה עיר ימים', 99, '+6%'], ['חיפה נווה שאנן', 94, '+2%'], ['אשדוד סיטי', 81, '-7%']]
const cockpit = (h, send) => h('div:max-w-5xl mx-auto px-5 sm:px-6 pt-0 sm:pt-2 pb-44', {},
  h('div:rounded-lg bg-white border border-gray-200 p-4 sm:p-6 shadow-xl overflow-hidden', {},
    h('div:flex flex-col lg:flex-row gap-6 lg:items-start lg:justify-between', {},
      h('div:max-w-2xl', {},
        h('div:inline-flex items-center gap-3 rounded-lg bg-[#F7FAF3] border border-[#61A60E]/20 px-4 py-2', {}, h('img:h-10 max-w-40 object-contain', { src: marketLogo, alt: 'מחסני השוק' }), h('span:text-xs font-bold text-[#4a7f0b]', {}, 'Retail cockpit')),
        h('h1:mt-5 text-3xl sm:text-5xl font-bold leading-tight text-[#17351f]', {}, 'בוקר טוב מחסני השוק'),
        h('p:mt-3 text-sm sm:text-base leading-7 text-gray-600', {}, 'תמונת מצב עדכנית מהרשת: קצב מכירות, סל ממוצע, מלאי בסיכון ורווחיות מבצעים מתוך Comax.')),
      h('div:w-full lg:max-w-sm rounded-lg border border-gray-200 bg-[#F8FAFC] p-4', {},
        h('div:flex items-center justify-between text-sm', {}, h('span:font-bold text-gray-900', {}, 'קצב מול יעד יומי'), h('span:rounded-full bg-[#61A60E] px-2 py-1 text-xs font-bold text-white', {}, 'חי')),
        h('svg:w-full h-32 mt-4 overflow-visible', { viewBox: '0 0 320 120', role: 'img', 'aria-label': 'מגמת פדיון עולה' },
        ...
        h('div:grid grid-cols-7 gap-1 text-center text-[10px] text-gray-500', { dir: 'ltr' }, trend.map(([t]) => h('span', { key: t }, t))))),
    h('div:grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-5', {}, marketKpis.map(([label, value, hint, icon, color]) =>
      h('div:rounded-lg bg-[#F8FAFC] border border-gray-200 p-4 text-[#17351f]', { key: label },
    ...
    h('div:grid grid-cols-1 lg:grid-cols-2 gap-4 mt-5', {},
      h('div:rounded-lg bg-[#F8FAFC] border border-gray-200 p-4', {},
        h('div:text-sm font-bold text-gray-900 mb-3', {}, 'סניפים מול יעד יומי'),
        branchBars.map(([name, value, delta]) => h('div:flex items-center gap-2 text-xs mb-2 last:mb-0 text-gray-700', { key: name },
      ...
      h('div:grid grid-cols-1 gap-2', {}, marketQuestions.map(([title, hint, q]) =>
        h('button:text-right rounded-lg bg-white border border-gray-200 px-4 py-3 text-[#17351f] shadow-sm hover:bg-[#F7FAF3] transition-colors', { key: title, onClick: () => send(q) },
