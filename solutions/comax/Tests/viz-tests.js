...
const {
  common: {
    boolean: { contains, notContains, and, equals },
  },
  react: {
    'react-comp': { AnalyticsAssistantResponse },
  },
  test: {
    Test,
...
const campaignRows = [
  { name: '120242290250740237', value: 410572 },
  { name: '39911 Open', value: 409001 },
  { name: '39912 Open', value: 137215 },
  { name: '39951', value: 106214 },
  { name: '39942 Open', value: 58746 },
  { name: '439951', value: 57321 },
]
...
Test('vizFlow.emitsPieWithParams', {
  impl: dataTest({
    calculate: runFlow(
      flowEmitting('{ kind: "pie", title: "Messages by sender", highlight: {name:"Alice", note:"most active"}, data: ($rows | map({name:.name, value:.value})) }'),
    ),
    expectedResult: and(
...
Test('vizFlow.emitsBarHighlightMax', {
  impl: dataTest({
    calculate: runFlow(
      flowEmitting('{ kind: "bar", title: "Senders", valueFormat: "int", highlight: {max:true, note:"top sender"}, data: ($rows | map({name:.name, value:.value})) }'),
    ),
    expectedResult: and(
...
const explorableElement = {
  ...
  rows: [
    { campaign: 'Brand', revenue: 18400 },
    { campaign: 'Search', revenue: 9200 },
  ],
  widgets: [
    {
      kind: 'bar',
      ...
      valueFormat: '$',
      data: [
        { name: 'Brand', value: 18400 },
        { name: 'Search', value: 9200 },
      ],
      drill: { dimension: 'campaign', question: 'Break {name} down by device' },
    },
  ],
...
Test('reactTest.applet.explorableAnswer', {
  impl: reactTest({
    testedComp:
      (ctx, { react: { hh } }) =>
      () =>
        hh(ctx, AnalyticsAssistantResponse, { element: explorableElement, send: () => {} }),
    expectedResult: and(
