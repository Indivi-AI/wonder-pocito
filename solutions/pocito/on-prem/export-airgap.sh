...
cd "$root"
git bundle create "$bundle_next" --all
mv "$bundle_next" wonder-pocito.bundle
docker build --platform linux/amd64 --target pocito-dev -f solutions/pocito/on-prem/on-premp-dev.dockerfile -t pocito-dev:linux-amd64 .
docker build --platform linux/amd64 --target pocito-dev-sudo -f solutions/pocito/on-prem/on-premp-dev.dockerfile -t pocito-dev:sudo-linux-amd64 .
