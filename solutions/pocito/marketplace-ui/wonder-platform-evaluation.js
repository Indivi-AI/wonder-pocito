    ...
    hFunc: (ctx, {react: {h}}) => ({repo, search, setSearch, openSet, createSet, runningSet, runSet}) => {
      ...
      const row = evaluation => {
        const run = lastRun(evaluation), linkedTarget = target(evaluation), cell = (label, content) => h(
          'div:flex items-center justify-between gap-3 md:block', {}, h('span:text-xs text-[#6b6b6f] md:hidden', {}, label), content)
        return h('div:grid grid-cols-1 items-center gap-3 border-t border-[#e8e8ea] px-4 py-4 md:grid-cols-[2fr_80px_140px_1.4fr_90px_100px]', {
          key: evaluation.id}, h('button:text-right', {onClick: () => openSet(evaluation)}, h('b:block text-sm', {}, evaluation.name),
        h('small:text-[#6b6b6f]', {}, evaluation.desc)), cell('רשומות', h('span:text-sm', {}, evaluation.rows.length)),
      ...
      return h('main:min-h-screen overflow-x-hidden px-5 pb-24 pt-8 sm:mr-[248px] sm:px-10', {}, h('div:mx-auto max-w-6xl', {},
        h('div:flex flex-wrap items-start justify-between gap-4', {}, h('div', {}, h('h1:text-2xl font-bold', {}, 'סטי אבלואציה'),
          h('p:mt-2 text-sm text-[#6b6b6f]', {}, 'ספרייה של תרחישי בדיקה, לשימוש חוזר מכל פלאגין או סאב-אייג׳נט.')),
      ...
      h('span:text-xs text-[#6b6b6f]', {}, `${sets.length} סטים`)), h('div:mt-5 rounded-2xl border border-[#e8e8ea] bg-white', {},
        h('div:hidden grid-cols-[2fr_80px_140px_1.4fr_90px_100px] gap-3 bg-[#fafafa] px-4 py-3 text-xs text-[#6b6b6f] md:grid', {},
