...
class S3ObjectStore:
    def __init__(self, bucket=None, client=None):
        ...
        self.client.meta.events.register('before-sign.s3.PutObject',
          lambda request, **_: request.headers.__delitem__('Expect') if 'Expect' in request.headers else None)
        try:
...
class MarketplaceRepository:
    ...
    def manifest_key(self, room, kind, name):
        return f'{safe_name(room)}/{kind}s/{safe_name(name)}/manifest.json'

    def version_key(self, room, kind, name, version):
        return f'{safe_name(room)}/{kind}s/{safe_name(name)}/versions/{version}.json'

    def load(self, key):
        try:
            return json.loads(self.objects.get(key))
        except FileNotFoundError:
            return None
        except json.JSONDecodeError:
            raise HTTPException(422, f'{key} is corrupt')

    def save(self, key, data, if_absent=False):
        self.objects.put(key, json.dumps(data, ensure_ascii=False).encode(), 'application/json', if_absent)

    def row(self, room, kind, name):
        safe_name(name)
        row = self.load(self.manifest_key(room, kind, name))
        if not row:
    ...
    def list(self, room, kind):
        ...
        rows = [self.load(key) for key in self.objects.list(f'{safe_name(room)}/{kind}s/') if key.endswith(suffix)]
        return [self.public(room, kind, row) for row in sorted(filter(None, rows), key=lambda item: item['display_name'])]
    ...
    def create(self, room, kind, payload):
        ...
        if self.load(self.manifest_key(room, kind, name)):
            raise HTTPException(409, f'{kind}/{name} already exists')
        data['_artifacts'] = self.artifacts(room, kind, name, 1, data)
        try:
            self.save(self.manifest_key(room, kind, name),
              data | {'version': 1, 'created_at': timestamp, 'updated_at': timestamp}, if_absent=True)
    ...
    def update(self, room, kind, name, changes):
        previous, timestamp = self.row(room, kind, name), now()
        ...
        merged = {key: value for key, value in previous.items() if key not in {'version', 'created_at', 'updated_at'}} | changes
        merged['display_name'] = name
        merged['_artifacts'] = self.artifacts(room, kind, name, version, merged, previous.get('_artifacts'))
        self.save(self.version_key(room, kind, name, previous['version']), previous)
        self.save(self.manifest_key(room, kind, name),
          merged | {'version': version, 'created_at': previous['created_at'], 'updated_at': timestamp})
        self.audit(room, kind, name, 'update', version, changes, timestamp)
        return self.get(room, kind, name)

    def delete(self, room, kind, name):
        row = self.row(room, kind, name)
        self.objects.delete_prefix(f'{safe_name(room)}/{kind}s/{safe_name(name)}/')
        self.audit(room, kind, name, 'delete', row['version'], {}, now())

    def audit(self, room, kind, name, action, version, data, timestamp):
        key = f'{safe_name(room)}/audit/{kind}/{safe_name(name)}/{timestamp}-{os.urandom(4).hex()}.json'
        self.save(key, {'action': action, 'version': version, 'data': data, 'ts': timestamp})

    def audits(self, room, kind, name):
        events = []
        for key in self.objects.list(f'{safe_name(room)}/audit/{kind}/{safe_name(name)}/'):
            try:
                events.append(json.loads(self.objects.get(key)))
            except json.JSONDecodeError:
                pass
    ...
    def versions(self, room, kind, name):
        ...
        prefix = f'{safe_name(room)}/{kind}s/{safe_name(name)}/versions/'
        rows = sorted((self.load(key) for key in self.objects.list(prefix)), key=lambda item: item['version'])
    ...
    def version(self, room, kind, name, version):
        row = self.load(self.version_key(room, kind, name, version))
        if not row:
    ...
    def file(self, room, kind, name, path):
        row = self.row(room, kind, name)
        item = row.get('_artifacts', {}).get(safe_path(path))
        if not item:
    ...
    def create_user(self, room, payload):
        ...
        self.save(f'{safe_name(room)}/users/{uid}.json', data, if_absent=True)
        return data

    def get_user(self, room, uid):
        key = f'{safe_name(room)}/users/{safe_name(uid)}.json'
        data = self.load(key)
        if not data:
    ...
    def agent_names(self):
        return [item['display_name'] for item in self.list(SCOPE, 'agent')]
...
def create_app(data_dir=None, model_factory=None, objects=None):
    data_dir = Path(data_dir or os.getenv('MARKETPLACE_DATA_DIR', ROOT / '.marketplace-data'))
    repo = MarketplaceRepository(objects or object_store())
