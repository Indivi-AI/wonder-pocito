...
ReactComp('wonderPlatformWorkspace', {
  impl: comp({
    hFunc: (ctx, {react: {h, hh, useEffect, useState}}) => props => {
      ...
      const createPending = repo.marketplace && !draft.originalId
      const createLabel = workspace.resource == 'plugins' ? 'יצירת פלאגין' : 'יצירת סוכן'
      ...
      h('span:text-xs text-[#8b948f]', {}, labels[workspace.resource]), h('input:min-w-0 flex-1 text-xl font-bold outline-none', {
        ...
        onBlur: () => !createPending && saveWorkspace(draft)}), h(`span:${classes.chip}`, {}, draft.version || 'V0'), createPending ? h(
          `button:${classes.primary}`, {disabled: !draft.name.trim() || !draft.id.trim(), onClick: () => saveWorkspace(draft)}, createLabel) : h(
