...
Data('wonderPlatformMinimaxV2Seed', {
  impl: () => {
    ...
    const ensureTags = items => items.map(x => Array.isArray(x.tags) ? x : { ...x, tags: x.tags || [] })
    return { version: 1, plugins: ensureTags(plugins), skills: ensureTags(skills), tools: ensureTags(tools),
      subagents: ensureTags(subagents), reports: ensureTags(reports), evaluations, conversations }
  }
})
Data('wonderPlatformMinimaxV2Ui', {
  impl: () => {
...
Data('wonderPlatformMinimaxV2RoomStore', {
  params: [
...
Data('wonderPlatformMinimaxV2Answer', {
  params: [
    {id: 'text', as: 'string'},
    {id: 'plugin', as: 'object'},
    {id: 'repo', as: 'object'},
    {id: 'history', as: 'array'},
    {id: 'selectedReports', as: 'array', defaultValue: []}
  ],
  impl: async (ctx, {}, {text, plugin, repo, history, selectedReports}) => {
    const started = Date.now(), extraReports = (selectedReports || []).filter(Boolean)
    ...
    return { text: output.text || result.workflowErrors?.[0]?.t || 'ההרצה הסתיימה ללא תשובה.', reportIds,
...
Booklet('wonderPlatformMinimaxV2', {
  impl: booklet('wonderPlatformMinimaxV2Assets,wonderPlatformMinimaxV2Response')
})
Doclet('wonderPlatformMinimaxV2Assets', {
  impl: `
...
Doclet('wonderPlatformMinimaxV2Response', {
  impl: `
...
Doclet('essentialOutputFormat.wonderPlatformMinimaxV2', {
  impl: `
...
Workflow('wonderPlatformMinimaxV2Agent', {
  params: [
...
%$wonderPlatformMinimaxV2%
Use the exact structured response flow and no unavailable component.`,
      thinkingBudget: 0
    }),
    categories: ['wonderPlatformMinimaxV2'],
    bookletsToLoad: ['wonderPlatformMinimaxV2']
  })
})
