...
ReactComp('wonderPlatformMinimaxV2Sidebar', {
  impl: comp({
...
ReactComp('wonderPlatformMinimaxV2Catalog', {
  impl: comp({
    hFunc: (ctx, { react: { h } }) => ({ repo, screen, query, setQuery, fileRef, importAssets, openEditor, ui, tagFilter, setTagFilter }) => {
      const allItems = repo[screen] || []
      const availableTags = Array.from(new Set(allItems.flatMap(x => x.tags || []))).sort()
      const items = allItems.filter(x => (!query || (x.name + ' ' + (x.desc || '')).includes(query))
        && (tagFilter.length === 0 || tagFilter.every(t => (x.tags || []).includes(t))))
      const toggleTag = tag => setTagFilter(tagFilter.includes(tag) ? tagFilter.filter(x => x !== tag) : [...tagFilter, tag])
      const clearTags = () => setTagFilter([])
      const chipBase = 'inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs cursor-pointer'
      const chipActive = 'border-[#c7e4d5] bg-[#e3f2ea] text-[#0a4a32]'
      const chipIdle = 'border-gray-200 bg-white text-[#6d7278]'
      ...
      h('div', { className: 'flex flex-col gap-2.5 px-8 pt-5 pb-2 max-md:px-4' },
        h('div', { className: 'flex items-center gap-2.5' }, h('label', { className: ui.classes.search },
        ...
        availableTags.length > 0 && h('div', { className: 'flex flex-wrap items-center gap-1.5', 'data-testid': 'tag-filter' },
          h('span', { className: 'text-xs text-gray-400' }, 'סינון לפי תגית:'),
          availableTags.map(tag => h('button', { className: chipBase + ' ' + (tagFilter.includes(tag) ? chipActive : chipIdle),
            key: tag, 'data-tag-chip': tag, onClick: () => toggleTag(tag) }, '#' + tag)),
          tagFilter.length > 0 && h('button', { className: chipBase + ' ' + chipIdle + ' underline', onClick: clearTags,
            'data-testid': 'tag-filter-clear' }, 'נקה'))),
      h('div', { className: 'grid grid-cols-[repeat(auto-fill,minmax(316px,1fr))] gap-4 px-8 pt-3.5 pb-10 max-md:grid-cols-1 max-md:px-4' },
        ...
        ), key: item.id, onClick: () => openEditor(screen, item), 'data-asset-id': item.id },
        ...
        h('p', {}, item.desc),
        (item.tags || []).length > 0 && h('div', { className: 'flex flex-wrap gap-1' },
          item.tags.map(tag => h('span', { className: ui.classes.neutralBadge + ' text-[10px]', key: tag, 'data-asset-tag': tag }, '#' + tag))),
        h('div', { className: 'mt-auto flex flex-wrap items-center gap-2 text-xs text-gray-400' },
...
ReactComp('wonderPlatformMinimaxV2Editor', {
  impl: comp({
    ...
    h('section', { className: 'rounded-2xl border border-gray-200 bg-white p-5 mb-4 grid grid-cols-2 gap-4 max-md:grid-cols-1' },
      ...
      h('label', { className: ui.classes.field + ' col-span-full max-md:col-span-1' }, h('span', {}, 'תגיות · מופרד בפסיק'),
        h('input', { 'data-testid': 'asset-tags-input', value: (draftAsset.tags || []).join(', '),
          placeholder: 'לדוגמה: חיוני, רבעון-נוכחי',
          onChange: e => setDraftAsset({ ...draftAsset,
...
ReactComp('wonderPlatformMinimaxV2Report', {
  impl: comp({
...
ReactComp('wonderPlatformMinimaxV2Trace', {
  impl: comp({
...
ReactComp('wonderPlatformMinimaxV2Chat', {
  impl: comp({
    hFunc: (ctx, { react: { h, hh } }) => props => {
      const { repo, activeConversation, activePlugin, busy, chatEndRef, message, setMessage, send, updateConversation, newConversation,
        setConversationId, toggleTrace, ui, compareSelection, toggleCompareSelection, compareReports } = props
      const verified = (repo.reports || []).filter(x => x.status === 'מאומת')
      return h('div', { className: 'h-screen flex min-h-0 max-md:h-[calc(100vh-4rem)]' },
        h('section', { className: 'flex-1 min-w-0 flex flex-col' },
          ...
          h('div', { className: 'flex-1 overflow-y-auto overflow-x-hidden flex flex-col gap-5 px-8 py-6 max-md:px-4' },
            ...
            verified.length >= 2 && h('section', { 'data-testid': 'compare-reports-picker',
              className: 'flex flex-col gap-2 rounded-2xl border border-[#c7e4d5] bg-[#f1faf4] p-4' },
              h('div', { className: 'flex items-center gap-2 text-[#0a4a32] text-sm font-semibold' },
                h('L:GitCompareArrows', { size: 16 }), 'השוואת דוחות מאומתים',
                h('span', { className: 'text-xs text-gray-500 font-normal' },
                  compareSelection.length + ' נבחרו · נדרשים לפחות 2')),
              h('div', { className: 'flex flex-wrap gap-1.5' }, verified.map(report => {
                const selected = compareSelection.includes(report.id)
                return h('button', { 'data-compare-report-id': report.id, 'data-selected': selected,
                  className: 'inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs cursor-pointer '
                    + (selected ? 'border-[#0e5c3f] bg-[#0e5c3f] text-white' : 'border-gray-200 bg-white text-[#6d7278]'),
                  key: report.id, onClick: () => toggleCompareSelection(report.id) },
                  h('L:BadgeCheck', { size: 12 }), report.name)
              })),
              h('div', { className: 'flex items-center gap-2' }, h('button', { className: ui.classes.primaryButton,
                'data-testid': 'compare-reports-submit',
                disabled: compareSelection.length < 2 || busy, onClick: compareReports },
                busy ? 'מריץ…' : ('השווה ' + compareSelection.length + ' דוחות')))
            ),
            (activeConversation?.messages || []).map(item => item.role === 'user' ? h('div', { className: (
              ...
              hh(ctx, dsls.react['react-comp'].wonderPlatformMinimaxV2Trace, { item, toggleTrace, ui }),
              ...
              (item.reportIds || []).map(id => {
                ...
                return report && hh(ctx, dsls.react['react-comp'].wonderPlatformMinimaxV2Report, { report, ui, key: id })
              }),
          ...
          hh(ctx, dsls.react['react-comp'].wonderPlatformMinimaxV2Composer, {
...
ReactComp('wonderPlatformMinimaxV2Composer', {
  impl: comp({
...
ReactComp('wonderPlatformMinimaxV2Evaluations', {
  impl: comp({
