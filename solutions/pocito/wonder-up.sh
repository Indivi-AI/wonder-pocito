...
#   ./wonder-up.sh --flapi           # also run the vendored FLAPI mock (flow-package tools) and point the stack at it
#   ./wonder-up.sh --clean           # stop everything and wipe volumes (fresh state)
set -euo pipefail
cd "$(dirname "$0")/on-prem"
MODE=dev ENVFILE="" FLAPI=""
while [[ $# -gt 0 ]]; do case "$1" in
  --airgap) MODE=airgap;; --clean) MODE=clean;; --flapi) FLAPI=1;; --env) ENVFILE="$2"; shift;; *) echo "unknown flag: $1" >&2; exit 1;;
esac; shift; done
...
profiles=(--profile local-minio); [[ -z "$FLAPI" ]] || profiles+=(--profile local-flapi)
if [[ "$MODE" == clean ]]; then compose -f compose.dev.yml --profile local-minio --profile local-flapi down -v; exit 0; fi
...
if ! grep -qE '^[A-Z_]*_PUBLISHED_PORT=' .env.site; then   # published ports: shift the 58045-58051 block +100 while another container holds one
  taken="$(docker ps --format '{{.Label "com.docker.compose.project"}}|{{.Ports}}' | grep -v '^wonder-onprem|' || true)"
  offset=0
  while grep -qE ":($((58045+offset))|$((58046+offset))|$((58047+offset))|$((58048+offset))|$((58049+offset))|$((58050+offset))|$((58051+offset)))->" <<< "$taken"
    do offset=$((offset+100)); done
  [[ "$offset" == 0 ]] || for entry in WONDER:58045 MARKETPLACE:58046 LLM_LITE:58047 MINIO:58048 AGNO:58049 PGVECTOR:58050 FLAPI:58051; do
    echo "${entry%:*}_PUBLISHED_PORT=$(( ${entry#*:} + offset ))" >> .env.site; done
fi
if [[ -n "$FLAPI" ]]; then   # point the stack at the local mock unless a real FLAPI is already configured
  fhost="$(sed -n 's/^SITE_HOST=//p' .env.site)"; fport="$(sed -n 's/^FLAPI_PUBLISHED_PORT=//p' .env.site)"
  grep -Eq '^FLAPI_BASE_URL=[^ ]' .env.site \
    || { sed -i.bak "s|^FLAPI_BASE_URL=.*|FLAPI_BASE_URL=http://$fhost:${fport:-58051}|" .env.site && rm -f .env.site.bak; }
  grep -Eq '^FLAPI_BEARER_TOKEN=[^ ]' .env.site \
    || { sed -i.bak "s|^FLAPI_BEARER_TOKEN=.*|FLAPI_BEARER_TOKEN=mock-test-token|" .env.site && rm -f .env.site.bak; }
fi

[[ "$MODE" == dev ]] && export PLATFORM="${PLATFORM:-$(docker version -f '{{.Server.Os}}/{{.Server.Arch}}')}" \
  WONDER_UID="$(id -u)" WONDER_GID="$(id -g)"
docker image inspect wonder-server-base:latest marketplace-server-base:latest wonder-flapi-mock:latest > /dev/null 2>&1 || ./build-images.sh --base
