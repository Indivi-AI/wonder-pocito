...
const customerPortfolioCompiledSql = `WITH base AS (
  ...
  FROM cols_cache(['signedRoom://finance3/usersRO/silver/transactions-18m-hist.parquet']) t
  LEFT JOIN cols_cache(['signedRoom://finance3/usersRO/silver/products.parquet']) p USING (product)
  LEFT JOIN cols_cache(['signedRoom://finance3/usersRO/silver/payments.parquet']) m USING (payment_method)
)
  ...
  impl: comp(
    ...
    )),
    {
      ...
      hFunc: (ctx, { reveal, react: { h, hh, useEffect, useRef, useState }, metadataLayout, tableZoomViews }) => () => {
        ...
        useEffect(() => {
          let active = true
          wfetch2('signedRoom://finance3/usersRO/silver/customer-portfolio.json', {}, ctx)
