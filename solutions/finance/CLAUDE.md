...
URLs: `localhost:3000/room/finance-demo/applet/FinanceDemo` (`?noAuth` skips login; local dev mints a wonder token) · same path on w-staging.indivi.ai.
...
- Data (finance-cube.js): `DATA_ROOT = room://finance-demo/usersRO/data` in EVERY realm; GCS is the source of truth.
  Linux lambdas use the cols_cache extension (`colsCache`); other realms mirror whole files to `/tmp/fullFileCache`.
  `resolveFroms` pins `db:'gcs'`. Browser queries use DuckDB-Wasm and `/gcs-proxy`; `?engine=lambda` uses the room lambda.
