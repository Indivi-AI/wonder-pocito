...
if [[ "${1:-}" == --base ]]; then
  ...
  docker build --platform "$PLATFORM" -f cloud-services/on-prem/marketplace-server-base.docker -t marketplace-server-base:latest solutions/idf/marketplace-server
fi
docker build --platform "$PLATFORM" --network=none -f cloud-services/on-prem/wonder-server.docker -t "wonder-server:$STAMP" .
docker build --platform "$PLATFORM" --network=none -f cloud-services/on-prem/marketplace-server.docker -t "marketplace-server:$STAMP" solutions/idf/marketplace-server
