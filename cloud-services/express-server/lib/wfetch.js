...
export const setupWfetch = app => app.post('/wfetch', async (req, res) => {
  try {
    ...
    const method = rawMethod.toUpperCase(), baseCtx = new coreUtils.Ctx(), parsed = url && jb.wonderUtils.extractFromUrl(url, baseCtx)
    ...
    const signed = parsed.scope.id == 'signedRoom', policy = signed && await roomPolicy(parsed.roomId)
