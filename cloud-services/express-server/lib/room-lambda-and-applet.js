// Room serving + running: the inline applet render shell AND the room-gated, runs-as-user lambda/snippet runner.
...
//  - GET  /room/:roomId/applet/:name → serve the render shell INLINE (no 302). Same URL for public & signed rooms; policy picks the teeth.
...
ctx.vars.roomLogger?.info?.({ t: 'serve applet shell', roomUrl, cmpId, urlsToLoad, appletV: appletSpec.appletV, uiSource }, {}, { ctx })
...
// serveAppletShell: turn an appletSpec ({cmpId, urlsToLoad, roomUrl, appletV, og}) into the inline shell. appletV = the share
...
export async function serveAppletShell(spec, res, localImports) {
  res.set({
...
export function setupRoomLambdaAndApplet(app) {
  ...
  const roomLambdaGate = async (req, res) => {
    ...
    const { who, policy, role, authError, authTiming } = await authorize(req, roomId)   // 1. authenticate
    if (authError) { res.status(401).json({ error: authError }); return null }
  ...
  // entry gate: SERVE the render shell INLINE (keeps the pretty /room/.../applet URL — no 302).
  // scheme is a property of the ROOM (has admin/users.json ⇒ signed), not of this GET's identity — a browser top-level nav
  // carries no x-user-authorization header (its token lives in localStorage.auth2, used later on the lambda POST). The shell
  ...
  app.get('/room/:roomId/applet/:name', async (req, res) => {
    try {
      ...
      await serveAppletShell(spec, res)
...
async function sourceImports(lambdaV, source, ctx) {
  if (source.userVars?.isLocalHost && source.profile) {
    ...
    const imp = await coreUtils.calcImportsForProfile(source.profile, { entryPointPaths: [developerEntryPoint()], ctx })
