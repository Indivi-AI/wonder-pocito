...
AsyncResult ParquetReader::Scan(ClientContext &context, ParquetReaderScanState &state, DataChunk &result) {
	...
	if (state.current_group < 0 || (int64_t)state.offset_in_group >= GetGroup(state).num_rows) {
		...
		// COLS_CACHE_RANGE_PLAN_BEGIN
		Value active_loggers;
		if (!row_group_pruned && StringUtil::StartsWith(file.path, "colscache://") &&
		    context.TryGetCurrentSetting("cols_cache_loggers", active_loggers) &&
		    active_loggers.ToString().find("colsCacheLogger") != string::npos) {
			std::ostringstream plan;
			plan << R"({"kind":"log","logger":"colsCacheLogger","channel":"info","event":{"src":"cpp","t":"scan.rangePlan","file":")"
			     << file.path.substr(strlen("colscache://")) << R"(","rg":)" << state.group_idx_list[state.current_group] << R"(,"ranges":[)";
			bool first = true;
			for (idx_t i = 0; i < column_ids.size(); i++) {
				auto file_col_idx = column_ids[MultiFileLocalIndex(i)];
				if (file_col_idx >= group.columns.size()) continue;
				auto &meta = group.columns[file_col_idx].meta_data;
				auto off = meta.__isset.dictionary_page_offset && meta.dictionary_page_offset > 0
				             ? meta.dictionary_page_offset : meta.data_page_offset;
				plan << (first ? "" : ",") << R"({"off":)" << off << R"(,"len":)" << meta.total_compressed_size << "}";
				first = false;
			}
			std::cerr << plan.str() << "]}}\n";
		}
		// COLS_CACHE_RANGE_PLAN_END
		if (!row_group_pruned) {
