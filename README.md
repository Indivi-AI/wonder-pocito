...
./solutions/pocito/wonder-up.sh --env ~/Downloads/team.env   # the .env.site your team lead sent (LLM keys; everything else auto-fills)
```
...
1. Clone and start **FlowBolt** (runs on port `6001` by default):
   ...
   ```
2. Configure the FLAPI environment variables in your `solutions/pocito/on-prem/.env.site` file:
   ```env
...
- `solutions/pocito/on-prem/README.md` — the air-gap deployment runbook (whitening kit, `--airgap` sim, OpenShift).
