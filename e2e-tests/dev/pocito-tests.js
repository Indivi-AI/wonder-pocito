...
import '../../ui/wonder-platform-runtime.js'
import '../../ui/wonder-platform.js'
import '../../ui/wonder-agents.js'
import './agno-test-runner.js'
...
Data('wonderPlatformWfetchRoundTrip', {
  ...
  impl: async (ctx, {}, {roomWUrl, listSkills}) => {
    ...
    const {createPocitoProdApp} = await import('../../infra/prod/main.js')
    const app = createPocitoProdApp(), server = await new Promise(resolve => {
    ...
    try {
      ...
      const write = await call({url: `${roomWUrl}/usersRW/wfetch-probe.json`, method: 'PUT', headers: {'content-type': 'application/json'}, body: {probe: true}})
      ...
      return {httpStatus: response.status, id: payload.body?.id, version: payload.body?.version,
        ...
        write: write.status, invalidUrl: invalidUrl.status}
...
Data('wonderPlatformFlapiRoundTrip', {
  impl: async () => {
    ...
    try {
      const {createFlapiApp} = await import('../../infra/dev/flapi-proxy.js')
...
Test('wonderPlatform.wfetchApi', {
  ...
  impl: dataTest({
    ...
    expectedResult: and(equals(200, '%httpStatus%'), equals('evidenceVerification.audit.he', '%id%'),
      ...
      equals(true, '%hasContent%'), equals(0, '%apiWorkflowErrors%'), equals(200, '%write%'), equals(400, '%invalidUrl%')),
    timeout: 30000,
    logger: 'workflowLogger'
  })
})
...
Test('wonderPlatform.marketplaceAgentCreate', {
  impl: reactTest(dsls.react['react-comp'].wonderPlatformMarketplaceTestApp(), and(contains('README (creation only)'), contains('שמירה')), {
    userActions: actions(waitForText('פלאגין ראיות'), click('סוכנים'), waitForText('סוכן חדש'), click('סוכן חדש'),
      waitForText('מי הסוכן'), click('הנחיות'), waitForText('README (creation only)'))})
})
...
Test('wonderPlatform.pluginGuidanceContinuesFromAgent', {
  impl: reactTest(dsls.react['react-comp'].wonderPlatformHomeTestApp(),
    ...
    userActions: actions(
      waitForText('מה נעשה היום?'), click('צור סוכן'), waitForText('מי הסוכן'),
      wonderPlatformSetControl({selector: '[aria-label="display_name"]', value: 'סוכן בדיקה'}),
...
Test('wonderPlatform.directConnectIsSecondary', {
  impl: reactTest(dsls.react['react-comp'].wonderPlatformHomeTestApp(),
    and(contains('מתקדם · חיבור ישיר'), contains('הוספת מיומנויות')), {
    userActions: actions(
      waitForText('מה נעשה היום?'), click('צור סוכן'), waitForText('מי הסוכן'),
      wonderPlatformSetControl({selector: '[aria-label="display_name"]', value: 'סוכן בדיקה'}),
      click('יכולות'), click('בניית פלאגין חדש'), waitForText('מי הפלאגין'),
