...
Data('pocitoOnPremDataset', {
  impl: async (ctx, {onPremLogger}) => {
    const {readFile} = await import('node:fs/promises'), {join} = await import('node:path'), {fileURLToPath} = await import('node:url')
    const data = fileURLToPath(new URL('../traveling-test/datasets/', import.meta.url))
...
Data('pocitoOnPremPgvectorRoundTrip', {
  impl: async (ctx, {onPremLogger}) => {
    ...
    const pocito = (await import('node:url')).fileURLToPath(new URL('../../', import.meta.url))
...
Data('pocitoOnPremMinioApplet', {
  impl: async (ctx, {onPremLogger}) => {
    ...
    const {urlsToLoad, shareImports} = uploadedManifest   // the def carries its share layout: alias -> served url under clientCodeWUrl
    const alias = Object.keys(shareImports).filter(key => urlsToLoad.startsWith(key)).sort((a, b) => b.length - a.length)[0]
    const sourceResponse = await jb.wonderUtils.wfetch2(
      `${published.clientCodeWUrl.replace(/\/?$/, '')}${shareImports[alias]}${urlsToLoad.slice(alias.length)}`, {}, dbCtx)
