...
const fixture = t => {
  const directory = mkdtempSync(join(solution, 'infra/export/images/.test-')), source = join(directory, 'source')
  const kit = join(source, 'infra/export/images'), exporter = join(source, 'infra/export/export-dev.sh')
  mkdirSync(kit, {recursive: true})
  t.after(() => rmSync(directory, {recursive: true, force: true}))
  cpSync(join(solution, 'infra/export/export-dev.sh'), exporter)
  cpSync(join(solution, 'README.md'), join(source, 'README.md'))
  writeFileSync(join(source, '.gitignore'), 'infra/export/images/\n')
...
test('code-only export clones and updates a real checkout, independently of image checksums', t => {
  ...
  const bundle = join(kit, 'wonder-pocito.bundle'), imageChecksums = readFileSync(join(solution, 'infra/export/images/SHA256SUMS'))
...
test('entrypoint preserves local changes, Git configuration, working directory and command arguments', t => {
  ...
  const result = run('sh', [join(solution, 'infra/dev/container/entrypoint.sh'), process.execPath, '-e',
  ...
  const missing = spawnSync('sh', [join(solution, 'infra/dev/container/entrypoint.sh'), 'true'],
...
test('every exported setup shell block parses as Bash', () => {
  const readme = readFileSync(join(solution, 'README.md'), 'utf8').split('## Air-gapped development container\n')[1]
