...
The ten local JPEGs live in `e2e-tests/traveling-test/assets/`. Their Wikimedia Commons source pages, authors, and licenses are recorded in
...
From `infra/dev/flapi-mock`:

```bash
npx tsx --test ../../../e2e-tests/traveling-test/flow-packages.test.ts
```
...
```bash
node ../../../e2e-tests/traveling-test/scripts/build-google-places.mjs \
  /tmp/traveling-test-overpass.json \
  ../../../e2e-tests/traveling-test/datasets/google-places.json
```
